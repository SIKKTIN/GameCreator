...
五份示例的 36 篇玩法文档已按内容主题归类，分类可在客户端继续改名、排序和调整。

| 原型 | 分类（文档数） |
| --- | --- |
| 空洞骑士 | 整体体验与进度（2）、角色能力与战斗（2）、地图探索与关卡（3）、敌人与首领（1） |
| 星露谷物语 | 生活节奏（2）、农场与种植（2）、物品与经济（1）、村庄与社交（1） |
| 植物大战僵尸 | 对局流程（2）、资源与布阵（1）、战斗与关卡（2） |
| 极乐迪斯科 | 探索与案件（2）、对话与检定（3）、角色与思想成长（2）、时间与日程（1） |
| 吸血鬼幸存者 | 对局与结算（2）、移动与战斗（3）、升级与装备构筑（3）、局外成长（1） |

分类仅调整文档归属，保留文档 ID、内容、编辑时间和所有跨模块引用。新建示例项目即带分类；已有项目不会因打开而自动改动。显式维护已有副本时可使用 `scripts/classify-prototype-documents.mjs` 的 `classifyPrototypeDocuments`：按稳定文档 ID 识别，只整理未分类的示例文档，保留已有自定义分类和自建文档，重复执行不会重复添加。
...
- mapDesign：可选 schema 1 地图设计。空洞骑士包含 4 个房间与 4 条物理双向通道（实际井口、梯子和独立往返条件），星露谷包含农场、村庄及双向连接；两者默认启用，空间实时引用原玩法。其他项目可在工作区设置中添加，参见[地图设计](../../docs/map-design.md)。
