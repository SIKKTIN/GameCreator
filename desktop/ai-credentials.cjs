...
async function issueWorkCredential(storage,input){
 ...
 if(!unique(permissions,3)||permissions.some(p=>!['progress','review','propose'].includes(p))||typeof workDescription!=='string'||workDescription.length>10000||typeof name!=='string'||!name.trim()||name.length>100||typeof expiresAt!=='string'||!Number.isFinite(Date.parse(expiresAt))||Date.parse(expiresAt)<=Date.now()||Date.parse(expiresAt)>Date.now()+366*86400000)throw new Error('令牌名称、权限、工作说明或有效期无效（最多一年）');
