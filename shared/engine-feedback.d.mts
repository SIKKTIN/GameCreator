...
export type FeedbackIdentity={verified:true;memberId:string;credentialId:string;memberName:string;intent:'dispatch'|'progress'|'review'|'propose'|'spec_change'|'project_change'};
export type Feedback={compatibility?:import('./project-standards.mjs').CompatibilityPlan;reason?:string;impact?:string;intent?:'dispatch'|'progress'|'review'|'propose'|'spec_change'|'project_change';identity?:{memberId:string;credentialId:string;signature:string};schema:1;projectId:string;engine:string;id:string;snapshotId:string;target:{kind:FeedbackKind;id:string};author:string;summary:string;evidence:string[];changes:Record<string,string>};
