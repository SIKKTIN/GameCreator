...
export function validateFeedback(value) {
  ...
  if(value.intent!==undefined&&!['progress','review','propose'].includes(value.intent))fail('反馈意图无效');
  ...
  for(const [key,v] of Object.entries(value.changes)) {
    if(!Object.hasOwn(feedbackFields[value.target.kind],key))fail('不支持回写字段 '+key);
...
export function validateFeedbackHistory(value,kind) {
  ...
  for(const r of value) {
    if(!record(r)||r.schema!==1||!feedbackIdPattern.test(r.id)||!snapshotIdPattern.test(r.digest)||!snapshotIdPattern.test(r.snapshotId)||!text(r.projectId,1200)||!r.projectId||!['godot-gdscript','oasis-lua'].includes(r.engine)||!['applied','dismissed'].includes(r.outcome)||!record(r.target)||r.target.kind!==kind||!text(r.target.id,1200)||!r.target.id||!text(r.title,3000)||!text(r.at,50)||!Number.isFinite(Date.parse(r.at))||!text(r.author,200)||!text(r.summary,3000)||!Array.isArray(r.evidence)||r.evidence.length>30||r.evidence.some(v=>!text(v,2000))||!Array.isArray(r.rows)||r.rows.length>4||!record(r.decisions)||seen.has(r.projectId+':'+r.id))return fail();
    if(r.identity!==undefined&&(!record(r.identity)||r.identity.verified!==true||!text(r.identity.memberId,200)||!text(r.identity.credentialId,200)||!text(r.identity.memberName,100)||!['progress','review','propose'].includes(r.identity.intent)))return fail();
    const fields=new Set();
    for(const row of r.rows) {
      if(!record(row)||!Object.hasOwn(feedbackFields[kind],row.field)||fields.has(row.field)||!text(row.label,100)||!['base','current'].every(k=>text(row[k],20*1024*1024))||!text(row.incoming,30000)||!['updated','unchanged','conflict'].includes(row.state)||!['keep','feedback'].includes(r.decisions[row.field]))return fail();
...
export function feedbackDiff(feedback,base,current) {
  ...
  return Object.entries(feedback.changes).map(([field,incoming])=>({field,label:feedbackFields[feedback.target.kind][field],base:base[field],current:current[field],incoming,
