...
(async()=>{
 ...
 try{
  ...
  await page.locator('.gp-links').getByRole('button',{name:'防守背景',exact:true}).click();assert.equal(await page.getByLabel('文档标题',{exact:true}).inputValue(),'防守背景');await page.getByLabel('文档标题',{exact:true}).fill('防守背景已改名');await nav('玩法设计');await page.locator('.gp-links').getByRole('button',{name:'防守背景已改名',exact:true}).waitFor();
  ...
  await app.evaluate(({ipcMain})=>{ipcMain.removeHandler('ai-documents-export');ipcMain.handle('ai-documents-export',(_event,payload)=>{payload={...payload,content:payload.files[0].content};globalThis.gameplayExport=payload;return {directory:'隔离测试导出',fileCount:payload.files.length,token:'test'};});});page.on('dialog',d=>d.accept());await nav('生成 AI 文档');await require('./ai-export-test-helper.cjs').finishAiExport(page);
  const exported=await app.evaluate(()=>globalThis.gameplayExport);assert.match(exported.folderName,/-AI文档-\d{4}-\d{2}-\d{2}-\d{6}$/);assert.equal(exported.files[0].path,'项目完整文档.md');for(const phrase of ['## 玩法设计','抵挡一波敌人','建造防御','正式美术与成长系统','通过首次试玩','防守背景已改名'])assert.ok(exported.content.includes(phrase),phrase);
  ...
  await nav('工程同步');await page.getByRole('textbox',{name:/^项目目录/}).fill(engine);await nav('保存设置');await page.waitForFunction(()=>!document.querySelector('.ps-trigger').disabled);assert.equal(storage.getItem(key(a)),aBeforeSwitch);
