...
(async()=>{
 ...
 try{
  ...
  await page.screenshot({path:path.join(qa,'pvz-material-characters.png')});await click('打开素材条目：向日葵场上形象与产出动作');await page.getByLabel('需求名称',{exact:true}).waitFor();await page.getByRole('tab',{name:'工程交付与进度',exact:true}).click();assert.equal(await page.getByLabel('素材工程交付路径',{exact:true}).count(),1);
  ...
  assert.deepEqual(JSON.parse(storage.getItem(key)).assets.map(a=>a.versions),JSON.parse(before).assets.map(a=>a.versions));assert.deepEqual(errors,[]);console.log('PASS PVZ material scope: three plant requirements, two enemies, design reference links, four shared animation packages, read-only browsing.');
