...
(async()=>{
 ...
 try{
  ...
  await click('进入本地工作区');await click('素材资产');await click('打开素材分类：角色');assert.equal(await page.getByRole('button',{name:/^打开素材需求：/}).count(),3);
  for(const name of ['向日葵场上形象与产出动作','豌豆射手形象与发射动作','坚果墙阻挡形象与受压表现'])await page.getByRole('button',{name:'打开素材需求：'+name,exact:true}).waitFor();
  await page.screenshot({path:path.join(qa,'pvz-material-characters.png')});await click('打开素材需求：向日葵场上形象与产出动作');await page.getByLabel('需求名称',{exact:true}).waitFor();assert.ok((await page.locator('.ar-reference-heading').allTextContents()).some(t=>t.includes('向日葵角色定型与配色基准')));
  await page.locator('.ar-reference-heading').getByRole('button',{name:'向日葵角色定型与配色基准',exact:true}).click();assert.equal(await page.getByLabel('资产名称',{exact:true}).inputValue(),'向日葵角色定型与配色基准');
  await click('素材分类');await click('打开素材分类：敌人与首领');await page.getByRole('tab',{name:/制作需求/}).click();assert.equal(await page.getByRole('button',{name:/^打开素材需求：/}).count(),2);await page.screenshot({path:path.join(qa,'pvz-material-enemies.png')});
  await click('素材分类');await click('打开素材分类：动画');await page.getByRole('tab',{name:/资产文件/}).click();assert.equal(await page.getByRole('button',{name:/^打开素材资产：/}).count(),4);
