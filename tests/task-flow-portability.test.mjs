...
for (const [i, example] of examples.entries()) test(slugs[i] + ': valid tasks, independent copies, portable archives and AI export retain every goal and branch', () => {
  ...
  const markdown = buildAiMarkdown(snapshot.document.archives.project, example.stories, example.data, example.definitions, config, { scan: null, active: null }, example.gameplay.designs, example.functionalSystems, example.artAssets, example.gameplayCore, example.taskFlows);
