...
test('art dispatch is signed, reviewed once and exported into member inboxes; wrong recipients and revoked grants fail',async t=>{
 const f=await fixture(t),s=rawRead(f.storage,f.sk),{presetPositions}=await import('../shared/ai-personnel.mjs');s.personnel={schema:1,positions:presetPositions('production'),members:[],credentials:[]};s.tasks[0].kind='美术';s.tasks[0].positionIds=['art'];f.storage.setItem(f.sk,JSON.stringify(s));
 const {createDeveloperService}=require('../desktop/ai-developers.cjs'),{signFeedback}=require('../shared/ai-feedback-client.cjs'),secrets=new Map();
 const service=createDeveloperService({storage:f.storage,vault:{put:v=>secrets.set(v.credentialId,v),get:(_p,id)=>secrets.get(id),remove:(_p,id)=>secrets.delete(id)}});
 const create=async(name,role,artPermissions=[])=>{const result=await service.change('create',{projectId:f.input.projectId,schedule:rawRead(f.storage,f.sk),name,duties:'制作',active:true,permissions:['progress','review'],profile:{positionIds:[role],taskIds:[],scope:'assigned',expiresAt:'',artPermissions}});return {...result,secret:service.read({projectId:f.input.projectId,credentialId:result.credential.id})};};
 const lead=await create('主美','art-director',['style','details','dispatch','propose']),artist=await create('美术开发','art'),coder=await create('程序','program');await f.sync();Object.assign(f.project,JSON.parse(await fs.readFile(path.join(f.engine,'gamecreator/project.json'),'utf8')));
 const post=async(to)=>{const feedback=signFeedback({...f.make({assigneeId:to}),intent:'dispatch'},lead.secret);await f.put(feedback);return feedback;};
 const bad=await post(coder.memberId);assert.match((await f.scan()).entries.find(e=>e.feedback?.id===bad.id||e.path.endsWith(bad.id+'.json')).error,/只能派发/);
 const feedback=await post(artist.memberId);let entry=(await f.scan()).entries.find(e=>e.feedback?.id===feedback.id);assert.equal(entry.state,'pending');assert.equal(feedbackBatchItems([entry]).ready.length,0);await assert.rejects(()=>f.apply(entry),/评估/);await f.apply(entry,{acceptProjectChange:true});
 let next=rawRead(f.storage,f.sk);assert.equal(next.tasks[0].assignment.primaryId,artist.memberId);assert.equal(next.tasks[0].dispatchHistory.length,1);assert.deepEqual(next.personnel.members.find(m=>m.id===artist.memberId).developer.taskIds,['task-a']);validateFeedbackHistory(next.feedbackHistory,'task');assert.equal((await f.scan()).entries.find(e=>e.feedback?.id===feedback.id).state,'processed');
 await f.sync();const inbox=JSON.parse(await fs.readFile(path.join(f.engine,'gamecreator/context/task-inbox.json'),'utf8'));assert.equal(inbox.members.find(m=>m.memberId===artist.memberId).tasks[0].id,'task-a');assert.match(await fs.readFile(path.join(f.engine,'gamecreator/members',lead.memberId+'.md'),'utf8'),/派发美术任务/);assert.ok(!JSON.stringify(inbox).includes(lead.secret.privateKey));
 Object.assign(f.project,JSON.parse(await fs.readFile(path.join(f.engine,'gamecreator/project.json'),'utf8')));const retry=await post(artist.memberId);entry=(await f.scan()).entries.find(e=>e.feedback?.id===retry.id);next=rawRead(f.storage,f.sk);next.personnel.members.find(m=>m.id===lead.memberId).developer.artPermissions=[];f.storage.setItem(f.sk,JSON.stringify(next));await assert.rejects(()=>f.apply(entry,{acceptProjectChange:true}),/变化|权限/);
});


test('engine art edits enforce granular permissions on both direct fields and parent replacements',async t=>{
 const f=await fixture(t),s=rawRead(f.storage,f.sk),{presetPositions}=await import('../shared/ai-personnel.mjs');s.personnel={schema:1,positions:presetPositions('production'),members:[],credentials:[]};f.storage.setItem(f.sk,JSON.stringify(s));
 const {createDeveloperService}=require('../desktop/ai-developers.cjs'),{signFeedback}=require('../shared/ai-feedback-client.cjs'),model=require('../desktop/project-content-model.cjs'),secrets=new Map();
 const service=createDeveloperService({storage:f.storage,vault:{put:v=>secrets.set(v.credentialId,v),get:(_p,id)=>secrets.get(id),remove:(_p,id)=>secrets.delete(id)}}),created=await service.change('create',{projectId:f.input.projectId,schedule:s,name:'技术美术',duties:'技术方案',active:true,permissions:['progress'],profile:{positionIds:['technical-art'],taskIds:[],scope:'assigned',expiresAt:'',artPermissions:['details','technical','propose']}}),secret=service.read({projectId:f.input.projectId,credentialId:created.credential.id});
 const ak='gamecreator.workspace.v1:'+f.input.projectId+':art-assets',art={schema:1,requirements:[{...model.authoringTemplates().requirement,id:'r'}],assets:[],links:[],style:model.authoringTemplates().artStyle};f.storage.setItem(ak,JSON.stringify(art));await f.sync();Object.assign(f.project,JSON.parse(await fs.readFile(path.join(f.engine,'gamecreator/project.json'),'utf8')));
 const post=async(changes)=>{const v=signFeedback({...f.make(changes),intent:'project_change',target:{kind:'module',id:'art-assets'},reason:'细化制作描述',impact:'保持既有引用',compatibility:{reuse:'保留条目',modify:'描述',add:'无',archive:'无'}},secret);await f.put(v);return (await f.scan()).entries.find(e=>e.feedback?.id===v.id||e.path.endsWith(v.id+'.json'));};
 let entry=await post({'/requirements/@r/description':JSON.stringify('拆图统一轴点')});assert.equal(entry.state,'pending');await f.apply(entry,{acceptProjectChange:true});assert.equal(rawRead(f.storage,ak).requirements[0].description,'拆图统一轴点');
 entry=await post({'/style/draft/direction':JSON.stringify('擅自更换美术风格')});assert.match(entry.error,/风格/);
 const rows=rawRead(f.storage,ak).requirements;rows[0].archived=true;entry=await post({'/requirements':JSON.stringify(rows)});assert.match(entry.error,/建议/);assert.equal(rawRead(f.storage,ak).requirements[0].archived,false);
});
