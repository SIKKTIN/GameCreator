...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('.pf-doc-list button').count(),18);
    ...
    await field('搜索框架规范').fill('枪械');assert.ok(await page.locator('.pf-doc-list button').count()<18);await field('搜索框架规范').fill('不存在的搜索结果abcdef');await page.getByText('没有找到匹配文档，试试其他关键词。').waitFor();await field('搜索框架规范').fill('');
    ...
    const files=JSON.parse(execFileSync('python',['-c',"import zipfile,json,sys;z=zipfile.ZipFile(sys.argv[1]);assert z.testzip() is None;print(json.dumps(z.namelist()))",zip],{encoding:'utf8'}));assert.equal(files.length,18);assert.ok(files.includes('通用游戏项目框架/README.md'));
    ...
    assert.deepEqual(errors,[]);console.log('PASS program framework: reader/tables/code/internal links, search navigation, 18-document ZIP, adoption and engine filtering, AI export, isolation/restart, draft retry and corrupt archive recovery.');
