...
(async()=>{
 ...
 try{
  ...
  assert.equal(await context().getByRole('menuitem',{name:'打开项目文件夹',exact:true}).evaluate(n=>n===document.activeElement),true);await page.keyboard.press('ArrowDown');assert.equal(await context().getByRole('menuitem',{name:'移除项目',exact:true}).evaluate(n=>n===document.activeElement),true);await page.keyboard.press('ArrowUp');await page.keyboard.press('Enter');await context().waitFor({state:'hidden'});await page.waitForFunction(()=>!document.querySelector('.ps-trigger').disabled);
  ...
  await open('尚无内容');await context().getByRole('menuitem',{name:'打开项目文件夹',exact:true}).click();await page.waitForFunction(()=>!document.querySelector('.ps-trigger').disabled);assert.deepEqual((await app.evaluate(()=>globalThis.folderCalls)).at(-1),{kind:'open',path:path.join(data,'storage')});
  ...
  await app.evaluate(({shell})=>{shell.openPath=async()=> 'QA folder unavailable';});await open('尚无内容');await context().getByRole('menuitem',{name:'打开项目文件夹',exact:true}).click();await page.getByRole('alert').filter({hasText:'QA folder unavailable'}).waitFor();assert.equal(JSON.parse(storage.getItem('gamecreator.projects.v1')).activeId,projects[0].id);
  await open('其他项目');await context().getByRole('menuitem',{name:'移除项目',exact:true}).click();const dialog=page.getByRole('dialog',{name:'从最近项目移除',exact:true});await dialog.waitFor();await dialog.getByRole('button',{name:'取消',exact:true}).click();assert.equal(JSON.parse(storage.getItem('gamecreator.projects.v1')).projects.length,3);
