...
test('full AI export, standalone rules, filtered engine delivery and search carry the same rules',()=>{
 ...
 const engine=syncDocuments(doc,['overview']);assert.ok(engine.find(f=>f.path==='modules/project-guide/standards.md').content.includes(section.body));assert.match(engine[0].content,/项目规范/);assert.match(engine.find(f=>f.path==='modules/overview.md').content,/更新项目前先读 \[项目规范\]\(project-guide\/standards.md\)/);
