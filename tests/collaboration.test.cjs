...
test('upgrade the original team database without rewriting existing stories or history', async () => {
  ...
  try {
    await service.close();
    const database = new DatabaseSync(path.join(directory, 'team.sqlite'));
    database.exec('ALTER TABLE stories DROP COLUMN details; DROP TABLE story_imports; ALTER TABLE users DROP COLUMN server_role; ALTER TABLE projects DROP COLUMN member_revision; DROP TABLE project_creations; DROP TABLE project_publications;');
    ...
    assert.equal(login.apiVersion, 11); assert.equal(login.user.serverRole, 'member'); assert.equal(read.stories.length, 2);
