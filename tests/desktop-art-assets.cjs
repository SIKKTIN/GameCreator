...
(async () => {
  ...
  const click = async name => {
    if(name.startsWith('打开素材资产：')){const asset=read().assets.find(a=>a.name===name.slice(7)),link=read().links.find(l=>l.assetId===asset.id),req=read().requirements.find(r=>r.id===link.requirementId);await page.getByRole('button',{name:'打开素材条目：'+req.name,exact:true}).click();await page.getByRole('tab',{name:'交付与版本',exact:true}).click();if(await field('当前交付资源').count())await field('当前交付资源').selectOption(asset.id);return;}
    if(name.startsWith('打开素材需求：'))name=name.replace('打开素材需求：','打开素材条目：');
  ...
  async function requirement(name){await click('新建素材条目');const dialog=page.getByRole('dialog',{name:'新建素材条目',exact:true});assert.equal(await dialog.getByRole('textbox').count(),1);await dialog.getByLabel('新素材名称',{exact:true}).fill(name);await dialog.getByRole('button',{name:'创建条目',exact:true}).click();return read().requirements.at(-1).id;}
  async function asset(name){if(!await page.locator('.ar-list-card.selected').count())await requirement(name);await page.getByRole('tab',{name:'交付与版本',exact:true}).click();await page.locator('.mi-reuse>summary').click();await field('新增交付资源名称').fill(name);await app.evaluate(({dialog})=>{dialog.showOpenDialog=async()=>({canceled:true,filePaths:[]});});await click('新建并导入交付资源');await page.waitForFunction(()=>!document.querySelector('.ar-version-dialog')?.open);return read().assets.at(-1).id;}
  ...
  try{
    ...
    await page.getByRole('tab',{name:'使用位置',exact:true}).click();await field('素材需求来源类型').selectOption('gameplay');await field('素材需求关联目标').selectOption(design.id);await field('素材需求具体来源').selectOption(JSON.stringify(['object',object.id]));await click('关联需求来源');
    ...
    await page.getByRole('tab',{name:'制作要求',exact:true}).click();await click('打开素材需求：豌豆射手动画需求');
    assert.equal(await field('需求状态').getByRole('option',{name:'已完成',exact:true}).evaluate(option=>option.disabled),true);assert.notEqual(read().requirements.find(x=>x.id===req).status,'已通过');assert.equal(await page.locator('.ps-trigger').isDisabled(),false);
    // Reuse one asset in a second requirement.
    const req2=await requirement('卡牌预览素材');await page.getByRole('tab',{name:'交付与版本',exact:true}).click();await page.locator('.mi-reuse>summary').click();await field('选择关联素材资产').selectOption(assetId);await click('关联素材资产');assert.equal(read().links.filter(x=>x.assetId===assetId).length,2);
    await click('打开素材资产：豌豆射手动画');await version('v2 正式动画',false);
    ...
    await page.getByRole('tab',{name:'制作要求',exact:true}).click();await click('打开素材需求：豌豆射手动画需求');await field('需求状态').selectOption('已通过');assert.equal(read().requirements.find(x=>x.id===req).status,'已通过');
    ...
    await page.getByRole('tab',{name:'使用位置',exact:true}).click();const sourceButton=page.getByRole('button',{name:/种植与射击.*空间对象/});await sourceButton.click();assert.equal(await field('对象名称').inputValue(),'豌豆射手站位');
    ...
    await app.evaluate(({ipcMain})=>{ipcMain.removeHandler('ai-documents-export');ipcMain.handle('ai-documents-export',(_event,payload)=>{payload={...payload,content:payload.files[0].content};globalThis.artExport=payload;return {directory:'隔离测试导出',fileCount:payload.files.length,token:'test'};});});page.on('dialog',d=>d.accept());await click('生成 AI 文档');await require('./ai-export-test-helper.cjs').finishAiExport(page);const exported=await app.evaluate(()=>globalThis.artExport);for(const value of ['素材需求','豌豆射手动画需求','侧视，待机与攻击','v1 灰盒占位','v2 正式动画','动作节奏与轮廓符合要求','pea-preview.png'])assert.ok(exported.content.includes(value),value);
    ...
    await fs.unlink(png);await fs.unlink(source);await app.close();app=null;await launch();await click('打开素材资产：豌豆射手动画');await click('预览交付文件：pea-preview.png');await page.locator('.ar-preview-image').waitFor();await page.waitForFunction(()=>document.querySelector('.ar-preview-image')?.naturalWidth>0);assert.equal(storage.getItem(key(a)),persisted);assert.equal(storage.getItem(gameKey),gameRaw);assert.equal(storage.getItem(funcKey),funcRaw);
    await app.close();app=null;storage.setItem(key(a),'{"schema":99}');await launch();assert.equal(await page.getByRole('button',{name:'新建素材条目',exact:true}).isDisabled(),true);assert.equal(await page.getByRole('button',{name:'生成 AI 文档',exact:true}).isDisabled(),true);assert.equal(storage.getItem(key(a)),'{"schema":99}');assert.deepEqual(errors,[]);
