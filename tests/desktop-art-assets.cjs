...
(async () => {
  ...
  async function launch(){app=await _electron.launch({executablePath:require('electron'),args:[path.join(root,'desktop/main.cjs')],env});page=await app.firstWindow();page.setDefaultTimeout(15000);page.on('pageerror',e=>errors.push(e.message));await click('登录');await click('美术资产');}
  async function selectProject(name){await page.locator('.ps-trigger').click();await page.getByRole('menuitemradio',{name:new RegExp('^'+name)}).click();await click('美术资产');}
  async function requirement(name){await page.getByRole('tab',{name:/^美术需求/}).click();await click('新建美术需求');const dialog=page.getByRole('dialog',{name:'新建美术需求',exact:true});assert.equal(await dialog.getByRole('textbox').count(),1);await dialog.getByLabel('新需求名称',{exact:true}).fill(name);await dialog.getByRole('button',{name:'创建需求',exact:true}).click();return read().requirements.at(-1).id;}
  async function asset(name){await page.getByRole('tab',{name:/^资产库/}).click();await click('新建美术资产');const dialog=page.getByRole('dialog',{name:'新建美术资产',exact:true});assert.equal(await dialog.getByRole('textbox').count(),1);await dialog.getByLabel('新资产名称',{exact:true}).fill(name);await dialog.getByRole('button',{name:'创建资产',exact:true}).click();return read().assets.at(-1).id;}
  ...
  try{
    ...
    await page.getByRole('tab',{name:/^美术需求/}).click();await click('打开美术需求：豌豆射手动画需求');await field('选择关联美术资产').selectOption(assetId);await click('关联美术资产');
    ...
    await page.getByRole('tab',{name:/^资产库/}).click();await click('打开美术资产：豌豆射手动画');await version('v2 正式动画',false);
    ...
    await page.getByRole('tab',{name:/^美术需求/}).click();await click('打开美术需求：豌豆射手动画需求');await field('需求状态').selectOption('已通过');assert.equal(read().requirements.find(x=>x.id===req).status,'已通过');
    // Forward and reverse references work from actual existing modules.
    await click('玩法设计');await click('打开玩法：种植与射击');await page.getByRole('region',{name:'关联美术需求',exact:true}).getByRole('button',{name:'查看美术需求：豌豆射手动画需求',exact:true}).click();assert.equal(await field('需求名称').inputValue(),'豌豆射手动画需求');
