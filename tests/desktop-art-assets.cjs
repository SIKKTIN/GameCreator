...
(async () => {
  ...
  async function launch(){app=await _electron.launch({executablePath:require('electron'),args:[path.join(root,'desktop/main.cjs')],env});page=await app.firstWindow();page.setDefaultTimeout(15000);page.on('pageerror',e=>errors.push(e.message));await click('登录');await click('美术资产');await click('查看全部内容');}
  async function selectProject(name){await page.locator('.ps-trigger').click();await page.getByRole('menuitemradio',{name:new RegExp('^'+name)}).click();await click('美术资产');await click('查看全部内容');}
  async function requirement(name){await page.getByRole('tab',{name:/^制作需求/}).click();await click('新建美术需求');const dialog=page.getByRole('dialog',{name:'新建美术需求',exact:true});assert.equal(await dialog.getByRole('textbox').count(),1);await dialog.getByLabel('新需求名称',{exact:true}).fill(name);await dialog.getByRole('button',{name:'创建需求',exact:true}).click();return read().requirements.at(-1).id;}
  async function asset(name){await page.getByRole('tab',{name:/^资产文件/}).click();await click('新建美术资产');const dialog=page.getByRole('dialog',{name:'新建美术资产',exact:true});assert.equal(await dialog.getByRole('textbox').count(),1);await dialog.getByLabel('新资产名称',{exact:true}).fill(name);await dialog.getByRole('button',{name:'创建资产',exact:true}).click();return read().assets.at(-1).id;}
  ...
  try{
    ...
    const req=await requirement('豌豆射手动画需求');await field('美术分类').selectOption('动画');await field('制作规格').fill('侧视，待机与攻击，透明背景');await field('美术验收标准').fill('攻击帧与发射事件对应');
    await field('美术需求来源类型').selectOption('gameplay');await field('美术需求关联目标').selectOption(design.id);await field('美术需求具体来源').selectOption(JSON.stringify(['object',object.id]));await click('关联需求来源');
    await field('美术需求来源类型').selectOption('capability');await field('美术需求关联目标').selectOption(capability.id);await click('关联需求来源');
    ...
    await page.getByRole('tab',{name:/^制作需求/}).click();await click('打开美术需求：豌豆射手动画需求');await field('选择关联美术资产').selectOption(assetId);await click('关联美术资产');
    ...
    const req2=await requirement('卡牌预览素材');await field('选择关联美术资产').selectOption(assetId);await click('关联美术资产');assert.equal(read().links.filter(x=>x.assetId===assetId).length,2);
    await page.getByRole('tab',{name:/^资产文件/}).click();await click('打开美术资产：豌豆射手动画');await version('v2 正式动画',false);
    ...
    await page.getByRole('tab',{name:/^制作需求/}).click();await click('打开美术需求：豌豆射手动画需求');await field('需求状态').selectOption('已通过');assert.equal(read().requirements.find(x=>x.id===req).status,'已通过');
    // Forward and reverse references work from actual existing modules.
    await click('玩法设计');await click('进入分类：未分类');await click('打开玩法：种植与射击');await page.getByRole('region',{name:'关联美术需求',exact:true}).getByRole('button',{name:'查看美术需求：豌豆射手动画需求',exact:true}).click();assert.equal(await field('需求名称').inputValue(),'豌豆射手动画需求');
    ...
    await click('功能系统');await click('打开功能：播放攻击动画');await page.getByRole('region',{name:'关联美术需求',exact:true}).getByRole('button',{name:'查看美术需求：豌豆射手动画需求',exact:true}).click();assert.equal(await field('需求名称').inputValue(),'豌豆射手动画需求');
    ...
    const saved=storage.getItem(key(a));await app.evaluate(({ipcMain},key)=>{globalThis.artStorageHandler=ipcMain.listeners('workspace-storage')[0];ipcMain.removeAllListeners('workspace-storage');ipcMain.on('workspace-storage',(event,request)=>{if(request?.operation==='set'&&request.key===key){event.returnValue={ok:false,error:'模拟美术存档写入失败'};return;}globalThis.artStorageHandler(event,request);});},key(a));
    await field('需求描述').fill('磁盘失败仍保留的草稿');assert.equal(storage.getItem(key(a)),saved);await page.getByRole('button',{name:'重试保存美术资产',exact:true}).waitFor();assert.equal(await page.locator('.ps-trigger').isDisabled(),true);assert.equal(await page.getByRole('button',{name:'生成 AI 文档',exact:true}).isDisabled(),true);assert.equal(await page.evaluate(()=>window.dispatchEvent(new Event('beforeunload',{cancelable:true}))),false);await click('退出登录');assert.equal(await field('需求描述').inputValue(),'磁盘失败仍保留的草稿');
    await app.evaluate(({ipcMain})=>{ipcMain.removeAllListeners('workspace-storage');ipcMain.on('workspace-storage',globalThis.artStorageHandler);});await click('重试保存美术资产');assert.equal(read().requirements.find(x=>x.id===req).description,'磁盘失败仍保留的草稿');
    await app.evaluate(({ipcMain})=>{ipcMain.removeHandler('write-markdown');ipcMain.handle('write-markdown',(_event,payload)=>{globalThis.artExport=payload;return '隔离测试导出';});});page.on('dialog',d=>d.accept());await click('生成 AI 文档');const exported=await app.evaluate(()=>globalThis.artExport);for(const value of ['美术需求','豌豆射手动画需求','侧视，待机与攻击','v1 灰盒占位','v2 正式动画','动作节奏与轮廓符合要求','pea-preview.png'])assert.ok(exported.content.includes(value),value);
    ...
    await fs.unlink(png);await fs.unlink(source);await app.close();app=null;await launch();await page.getByRole('tab',{name:/^资产文件/}).click();await click('打开美术资产：豌豆射手动画');await click('预览交付文件：pea-preview.png');await page.locator('.ar-preview-image').waitFor();await page.waitForFunction(()=>document.querySelector('.ar-preview-image')?.naturalWidth>0);assert.equal(storage.getItem(key(a)),persisted);assert.equal(storage.getItem(gameKey),gameRaw);assert.equal(storage.getItem(funcKey),funcRaw);
    await app.close();app=null;storage.setItem(key(a),'{"schema":99}');await launch();assert.equal(await page.getByRole('button',{name:'新建美术需求',exact:true}).isDisabled(),true);assert.equal(await page.getByRole('button',{name:'生成 AI 文档',exact:true}).isDisabled(),true);assert.equal(storage.getItem(key(a)),'{"schema":99}');assert.deepEqual(errors,[]);
