...
await field('文档目标目录').fill('res://design/reference');await field('素材目标目录').fill('art/delivery');await button('保存同步配置').click();await page.getByText('同步配置已保存。',{exact:true}).waitFor();assert.equal(await field('同步工程根目录').innerText(),engine);assert.equal(await field('文档最终写入位置').innerText(),engine+'/design/reference');assert.equal(await field('素材最终写入位置').innerText(),engine+'/art/delivery');
   ...
   // Changing the connection must change the actual write root, not just the displayed path.
   const other=path.join(dir,'second-engine');await fs.mkdir(other);await fs.writeFile(path.join(other,'project.godot'),'config_version=5');
   const oldManifest=await fs.readFile(path.join(engine,'.gamecreator-sync/manifest.json'),'utf8');
   await button('工程连接').click();await field('项目目录').fill(other);await button('工程同步').click();await page.getByText('工程连接有未保存的修改，请先保存工程连接。',{exact:true}).waitFor();assert.equal(await button('预览同步变更').isDisabled(),true);assert.equal(await field('同步工程根目录').innerText(),engine);
   await button('工程连接').click();await button('保存设置').click();await page.getByText('设置已保存。',{exact:true}).waitFor();await button('工程同步').click();assert.equal(await field('同步工程根目录').innerText(),other);assert.equal(await field('文档最终写入位置').innerText(),other+'/design/reference');
   await check();assert.ok((await page.locator('.es-changes').innerText()).includes(other+'/design/reference/README.md'));await button('同步到工程').click();await page.getByText(/文件已同步；引擎导入状态未检测，共/).waitFor();assert.match(await fs.readFile(path.join(other,'design/reference/modules/gameplay/gameplay.md'),'utf8'),/植物/);assert.deepEqual(await fs.readFile(path.join(other,asset.path)),await fs.readFile(source));assert.equal(await fs.readFile(path.join(engine,'.gamecreator-sync/manifest.json'),'utf8'),oldManifest);
