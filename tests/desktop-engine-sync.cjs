...
await field('文档目标目录').fill('res://design/reference');await field('素材目标目录').fill('art/delivery');await button('保存同步配置').click();await page.getByText('同步配置已保存。',{exact:true}).waitFor();assert.equal(await field('同步工程根目录').innerText(),engine);assert.equal(await field('文档最终写入位置').innerText(),engine+'/design/reference');assert.equal(await field('素材最终写入位置').innerText(),engine+'/art/delivery');
   ...
   assert.match(await fs.readFile(path.join(engine,'design/reference/config-data-policy.md'),'utf8'),/当前项目目录约定/);
   ...
   await tab('工程连接').click();await field('项目目录').fill(other);await tab('同步配置').click();await page.getByText('工程连接有未保存的修改，请先保存工程连接再同步。',{exact:true}).waitFor();assert.equal(await button('预览同步变更').isDisabled(),true);assert.equal(await field('同步工程根目录').innerText(),engine);
   await tab('工程连接').click();await button('保存设置').click();await page.getByText('设置已保存。',{exact:true}).waitFor();await tab('同步配置').click();assert.equal(await field('同步工程根目录').innerText(),other);assert.equal(await field('文档最终写入位置').innerText(),other+'/design/reference');
