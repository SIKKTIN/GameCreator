...
const examples = await Promise.all(['plants-vs-zombies', 'stardew-valley', 'hollow-knight'].map(async name => ({ name, example: JSON.parse(await fs.readFile(new URL('../examples/prototypes/' + name + '.json', import.meta.url), 'utf8')) })));
