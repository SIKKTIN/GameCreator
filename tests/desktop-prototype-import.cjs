...
(async () => {
  ...
  async function verifyModules(id, fixture) {
    ...
    await click('素材资产'); await click('素材分类'); await click('查看全部内容'); await page.getByRole('region', { name: '素材资产工作区', exact: true }).waitFor();
    assert.equal(await page.getByRole('button', { name: /^打开素材需求：/ }).count(), value.artAssets.requirements.length);
    await page.getByRole('tab', { name: /^资产文件/ }).click();
    assert.equal(await page.getByRole('button', { name: /^打开素材资产：/ }).count(), value.artAssets.assets.length);
  ...
  try {
    ...
    await chooseProject(imported[0].name); await click('素材资产'); await click('素材分类'); await click('查看全部内容');
    await page.getByRole('button', { name: '打开素材需求：' + fixtures[0].value.artAssets.requirements[0].name, exact: true }).click();
    ...
    await click('素材资产'); await click('素材分类'); await click('查看全部内容'); await page.getByRole('button', { name: '打开素材需求：' + fixtures[0].value.artAssets.requirements[0].name, exact: true }).click();
