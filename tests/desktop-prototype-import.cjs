...
(async () => {
  ...
  async function verifyModules(id, fixture) {
    ...
    assert.equal(await page.getByRole('button', { name: /^打开素材条目：/ }).count(), (await import('../src/material-items.ts')).materialItems(value.artAssets).length);
  ...
  try {
    ...
    await page.getByRole('button', { name: '打开素材条目：' + fixtures[0].value.artAssets.requirements[0].name, exact: true }).click();
    ...
    await click('素材资产'); await click('素材分类'); await click('查看全部内容'); await page.getByRole('button', { name: '打开素材条目：' + fixtures[0].value.artAssets.requirements[0].name, exact: true }).click();
