...
for(const file of ['hollow-knight','stardew-valley'])test(file+': live spatial references, all routes and conditions, layering and disabled data survive transfer',async()=>{
  ...
  memory.setItem('gamecreator.workspace.v1:'+a.project.id+':prototype-design',JSON.stringify(store));
  const snapshot=captureProjectPackage(memory,a.project);assert.deepEqual(snapshot.document.archives['prototype-design'],store);assert.deepEqual(snapshot.document.archives['map-design'],disabled);const restored=prepareProjectPackageImport(catalog,snapshot.document,'复原');writeProjectPackageImport(memory,restored);assert.deepEqual(JSON.parse(memory.getItem(key(restored.project.id))),disabled);
