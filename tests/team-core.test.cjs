...
test('core permission migration and legacy clients preserve overrides; streamed writes reauthorize after revocation',()=>fixture(async(ctx)=>{
  ...
  await restart(db=>db.exec('ALTER TABLE member_permissions DROP COLUMN core'));const updated=await login('bob');const migrated=await read(updated);assert.deepEqual(migrated.capabilities,{overview:'edit',stories:'view',core:'edit'});
}));
