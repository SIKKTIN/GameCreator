...
(async()=>{
 ...
 for(let i=0;i<85;i++)art.requirements.push({...structuredClone(art.requirements[0]),id:'search-return-'+i,name:'返回定位测试 '+String(i).padStart(2,'0'),description:'用于验证返回搜索时保留分页与滚动位置',generationPrompt:undefined,sources:[]});
 storage.setItem(artKey,JSON.stringify(art));
 ...
 async function noBack(){await back().waitFor({state:'detached'});}
 try{
 ...
 await noBack();await button('素材资产').click();await noBack();
 await search('冲刺');await button('玩法设计').click();await noBack();
 ...
 await button('素材资产').click();await noBack();
 await open(design.id,design.title,'玩法设计');await button('玩法设计').click();await noBack();
 await open(design.id,design.title,'玩法设计');await button('分类总览').click();await noBack();
 ...
 await back().waitFor();await button('查看节点：'+design.stateFlow.states[0].name).click();await noBack();
 ...
 await field('生成提示词').fill((await field('生成提示词').inputValue())+'\n验证编辑保留返回入口');await back().waitFor();
 await button('素材分类').click();await noBack();
 await open(req.id,req.name,'素材资产');await field('素材分类筛选').selectOption('all');await noBack();
 await open(req.id,req.name,'素材资产');await page.locator('.ar-list button').filter({hasNotText:req.name}).first().click();await noBack();
 await open(req.id,req.name,'素材资产');await page.locator('.ar-reference-heading .gp-link-name').first().click();await noBack();
 // A related result opened in the search preview has the same search origin.
 await search(req.id,'素材资产');await button('预览搜索结果：'+req.name).click();
 const preview=page.getByRole('complementary',{name:'搜索结果预览'});await preview.locator('.gsearch-related').first().click();await back().waitFor();await back().click();await preview.waitFor();assert.equal(await field('搜索当前项目').inputValue(),req.id);
 // Preserve the result snapshot even when the header text changes in the editor.
 await search('返回定位测试','素材资产');await field('包含已归档内容').check();
 await panel().getByRole('button',{name:/显示更多结果/}).click();assert.equal(await panel().locator('.gsearch-result').count(),80);
 await button('预览搜索结果：返回定位测试 60').click();
 const scrollTop=await panel().locator('.gsearch-results').evaluate(n=>{n.scrollTop=1800;return n.scrollTop;});
 await preview.getByRole('button',{name:'打开内容',exact:true}).click();await back().waitFor();
 await field('全局搜索入口').fill('尚未提交的新关键词');await page.waitForTimeout(200);assert.ok((await back().innerText()).includes('返回定位测试'));
 await back().click();await page.waitForTimeout(200);assert.equal(await field('搜索当前项目').inputValue(),'返回定位测试');
 assert.equal(await field('包含已归档内容').isChecked(),true);assert.equal((await panel().locator('.gsearch-filters button.active').innerText()).replace(/\s/g,''),'素材资产85');
 assert.equal(await panel().locator('.gsearch-result').count(),80);assert.ok((await preview.innerText()).includes('返回定位测试 60'));
 assert.equal(await panel().locator('.gsearch-results').evaluate(n=>n.scrollTop),scrollTop);
 await field('包含已归档内容').uncheck();await button('地图设计').click();await noBack();
 ...
 const analysis=example.numericalAnalysis.plans[0];await open(analysis.id,analysis.name,'数值分析');await button('参数与公式').click();assert.equal(await field('分析名称').inputValue(),analysis.name);
 await back().waitFor();await button('项目概览').click();await noBack();await button('数值分析').click();await noBack();
 ...
 assert.deepEqual(errors,[]);console.log('PASS desktop global search: module and item navigation, editing, related links, search snapshot/scroll restoration, all target types, narrow layout and project isolation.');
