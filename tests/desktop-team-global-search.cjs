...
(async()=>{
 ...
 const noBack=()=>back().waitFor({state:'detached'});
 ...
 await button('打开搜索结果：团队共享唯一玩法').click();await page.getByRole('textbox',{name:'玩法名称',exact:true}).waitFor();await back().waitFor();
 // Exercise the same cancellable event used by failed draft saves/in-flight saves.
 await page.evaluate(()=>{window.searchTestGuard=e=>e.preventDefault();window.addEventListener('gamecreator:leave-team',window.searchTestGuard);});
 await button('项目概览').click();await back().waitFor();await page.getByRole('textbox',{name:'玩法名称',exact:true}).waitFor();
 await button('分类总览').click();await back().waitFor();await page.getByRole('textbox',{name:'玩法名称',exact:true}).waitFor();
 await back().click();await back().waitFor();assert.equal(await panel().isVisible(),false);
 await page.keyboard.press('Control+k');assert.equal(await panel().isVisible(),false);await back().waitFor();
 await page.evaluate(()=>window.removeEventListener('gamecreator:leave-team',window.searchTestGuard));
 await button('分类总览').click();await noBack();
 await button('全局搜索').click();
 await page.evaluate(()=>window.addEventListener('gamecreator:leave-team',window.searchTestGuard));
 await button('打开搜索结果：团队共享唯一玩法').click();await noBack();assert.equal(await panel().isVisible(),true);
 await page.evaluate(()=>window.removeEventListener('gamecreator:leave-team',window.searchTestGuard));
 await button('打开搜索结果：团队共享唯一玩法').click();await back().waitFor();
 await button('玩法设计').click();await noBack();await button('全局搜索').click();
 await button('打开搜索结果：团队共享唯一故事').click();await page.locator('.shared-story-editor').waitFor();await back().waitFor();
 await button('项目概览').click();await noBack();await button('故事文档').click();await noBack();await button('全局搜索').click();
 // A later server deletion must disappear without switching projects.
 await button('打开搜索结果：团队共享唯一任务').click();await dialog.waitFor();await dialog.getByRole('button',{name:'关闭',exact:true}).click();await back().waitFor();
 const fresh=await api(base+'/schedule',token);await api(base+'/schedule',token,'PUT',{requestId:crypto.randomUUID(),changes:scheduleChanges(fresh,{...fresh.store,tasks:[]})});await noBack();await button('全局搜索').click();await button('打开搜索结果：团队共享唯一任务').waitFor({state:'detached'});
 await button('打开搜索结果：团队共享唯一故事').click();await back().waitFor();
 ...
 await noBack();await button('全局搜索').click();await page.getByText('当前项目不可访问，搜索结果已清除。',{exact:false}).waitFor();assert.equal(await panel().locator('.gsearch-result').count(),0);assert.equal(await panel().locator('.gsearch-preview').count(),0);assert.deepEqual(errors,[]);
 console.log('PASS team search: authorized coverage, cancelled navigation, same-module navigation, deletion of the open target and return/result clearing after access revocation.');
