...
(async()=>{
  ...
  try {
    ...
    await button('工程同步').click();await tab('开发反馈').click();await field('反馈处理 任务状态').waitFor();assert.equal(await button('应用反馈').isDisabled(),true);await field('反馈处理 任务状态').selectOption('feedback');
    ...
    await button('工程同步').click();await tab('开发反馈').click();await button('处理记录 1').waitFor();await page.getByText('没有待处理反馈',{exact:true}).waitFor();
    ...
    await post({kind:'task',id:task.id},{status:'已完成'},'待人工核实的完成反馈');await button('工程同步').click();await tab('开发反馈').click();await button('忽略本条反馈').click();await page.getByText(/反馈已忽略/).waitFor();assert.equal((await read(sk)).tasks.find(t=>t.id===task.id).status,'待验收');
    ...
    await tab('同步配置').click();await tab('开发反馈').click();await button('待处理反馈 6').waitFor();assert.match(await button('一键应用反馈').innerText(),/4/);await button('一键应用反馈').click();await page.getByText(/批量处理完成：已应用 4 条，保留待处理 2 条，失败 0 条/).waitFor();await button('处理记录 7').waitFor();await button('待处理反馈 2').waitFor();await button('读取开发反馈').waitFor();
