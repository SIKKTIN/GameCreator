...
(async()=>{
  ...
  async function scan(){await button('读取开发反馈').click();await button('读取开发反馈').waitFor();await page.waitForFunction(()=>!document.querySelector('.ef-panel .primary')?.disabled);}
  ...
  try {
    ...
    await button('引擎设置').click();await tab('开发反馈').click();await scan();await field('反馈处理 任务状态').waitFor();assert.equal(await button('应用反馈').isDisabled(),true);await field('反馈处理 任务状态').selectOption('feedback');
    ...
    await button('引擎设置').click();await tab('开发反馈').click();await scan();await page.getByText('没有待处理反馈',{exact:true}).waitFor();
    ...
    await post({kind:'task',id:task.id},{status:'已完成'},'待人工核实的完成反馈');await button('引擎设置').click();await tab('开发反馈').click();await scan();await button('忽略本条反馈').click();await page.getByText(/反馈已忽略/).waitFor();assert.equal((await read(sk)).tasks.find(t=>t.id===task.id).status,'待验收');
    await scan();await page.getByRole('button',{name:/处理记录 3/}).click();assert.equal(await page.locator('.ef-panel .es-history').count(),3);
    ...
    await app.close();app=null;await launch();await tab('开发反馈').click();await scan();await page.getByText('没有待处理反馈',{exact:true}).waitFor();assert.equal((await read(sk)).feedbackHistory.length,2);assert.equal((await read(tk)).feedbackHistory.length,1);
    ...
    console.log('PASS desktop feedback: folder-backed context export, field conflicts, acceptance, immediate task/tool refresh, dismissal, history, receipt repair, restart and narrow layout.');
