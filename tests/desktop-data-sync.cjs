...
(async()=>{
  ...
  try {
    ...
    await editManifest('18');await fs.writeFile(path.join(engine,'data/generated/manifest.json'),JSON.stringify({...manifest,spawn_count:19}));await button('数据同步').click();await preview();await field('同步 manifest').check();assert.equal(await button('应用导入（2）').isDisabled(),true);await field('manifest value/spawn_count 采用值').selectOption('remote');await button('应用导入（2）').click();await page.getByText(/导入完成，已保存/).waitFor();assert.equal((await read()).data.datasets.manifest.find(r=>r.id==='spawn_count').value,'19');
    ...
    console.log('PASS desktop data sync: filename import, typed object edit, export, explicit conflict resolution, automatic export, restart, narrow layout.');
