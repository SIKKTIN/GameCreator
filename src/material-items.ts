...
export function materialItems(store:ArtStore):MaterialItem[] {
  ...
  const build=(kind:ArtItemKind,source:ArtRequirement|ArtAsset,assets:ArtAsset[],requirement?:ArtRequirement):MaterialItem=>({
    ...
    status:requirement?.status||(assets.some(a=>a.adoptedVersionId)?'已有采用版本':assets.some(a=>a.versions.length)?'待审核':'待制作'),owner:requirement?.owner||'',dueDate:requirement?.dueDate||'',requirement,assets,
    files:assets.reduce((n,a)=>n+a.versions.reduce((s,v)=>s+v.files.length,0),0),versions:assets.reduce((n,a)=>n+a.versions.length,0),
    searchText:[source.name,source.description,requirement?.owner,requirement?.specification,requirement?.generationPrompt?.prompt,...assets.flatMap(a=>[a.name,a.description,...a.versions.flatMap(v=>[v.name,...v.files.map(f=>f.name)])])].join(' ').toLocaleLowerCase(),
  });
...
export function addMaterialRequirements(store:ArtStore,assetId:string) {
  ...
  const requirement=created.store.requirements.find(r=>r.id===created.target.id)!;requirement.description=asset.description;
