...
export function AiDeveloperList({controller:c,projectId,testMode,toolHistory=[]}:Props){
 ...
 const action=(operation:'copy'|'download'|'import'|'revoke'|'delete',key:AiCredential)=>run(async()=>{if(!api)throw new Error('请在桌面客户端管理凭证');if(operation==='revoke'&&!window.confirm('撤销此令牌？开发者资料与反馈记录保留，之后可以更换令牌。'))return;if(operation==='delete'&&!window.confirm('删除此令牌？该令牌将立即失效，并清理本机保存的对应凭证。开发者身份、任务及反馈历史保留。'))return;const result=await api(operation,{projectId,credentialId:key.id,schedule:c.store});if(result)op.setNotice(operation==='copy'?'完整令牌已复制。':operation==='download'?'凭证文件已保存。':operation==='import'?'凭证已验证并加密保存，现在可以重新复制。':operation==='delete'?'令牌已删除，开发者、任务和反馈历史保留。':'令牌已撤销，开发者与历史记录保留。');});
