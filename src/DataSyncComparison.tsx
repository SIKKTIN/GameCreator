import {useDataSyncBatch} from './DataSyncBatch';
...
export function DataSyncComparison({file, decisions, disabled, onChange}: Props) {
  ...
  const valueGroups = valueRows.map(row => ({id: row.id, label: row.label, localExists: true, remoteExists: true, differences: row.cells.flatMap(c => c.differences)}));
  const batch = useDataSyncBatch({scope: file.table + ' 值差异', groups: valueGroups, decisions, disabled, onChange});
  ...
  return <div className="ds-comparison" aria-label={file.table + ' 差异预览'}>
    ...
    {files.length ? <><StructureTable title="配置新增" entity="配置" groups={files} {...{file, decisions, disabled, onChange}} preventDelete/><p className="ds-comparison-hint">该配置仅在一侧存在，将整体添加，无需逐项比较值。</p></> : <>
      ...
      {values.length ? <>
        ...
        {batch.tools}
        <div className="ds-table-scroll"><table className="ds-compare-table ds-value-table" aria-label={file.table + ' 同名字段值差异'}><thead><tr><th scope="col">{batch.all}{file.shape === 'object' ? '配置' : '记录 ID'}</th><th scope="col">来源</th>{valueHeaders.map(h => <th scope="col" key={h.key}><code>{h.key}</code>{h.remoteKey !== h.key && <small>引擎：{h.remoteKey}</small>}</th>)}<th scope="col">采用值</th></tr></thead>
          <tbody>{valueRows.map(row => <Fragment key={row.id}>{(['local', 'remote'] as const).map((side, index) => <tr key={side} className={'ds-value-row ds-side-' + side}>
            {!index && <th scope="rowgroup" rowSpan={2} className="ds-record-id"><span className="ds-batch-row">{batch.checkbox(valueGroups.find(g => g.id === row.id)!)}<code>{row.label}</code></span></th>}<th scope="row" className="ds-source-label">{side === 'local' ? 'GameCreator' : '引擎文件'}</th>
        ...
        {cell && <section className="ds-cell-detail" aria-label="单元格差异详情"><div className="ds-section-heading"><h4><code>{cell.record} · {cell.key}</code></h4><button type="button" aria-label="关闭差异详情" onClick={() => setActive('')}><X size={15}/></button></div><DifferenceTable key={cell.id} differences={cell.differences} {...{file, decisions, disabled, onChange}}/></section>}
...
function StructureTable({title, entity, groups, preventDelete, ...props}: Props & {title: string; entity: string; groups: DifferenceGroup[]; preventDelete?: boolean}) {
  const batch = useDataSyncBatch({scope: props.file.table + ' ' + title, groups, preventDelete, ...props});
  return <>{batch.tools}<div className="ds-table-scroll"><table className="ds-compare-table ds-structure-table" aria-label={props.file.table + ' ' + title}><thead><tr><th scope="col">{batch.all}{entity}</th><th scope="col">GameCreator</th><th scope="col">引擎文件</th><th scope="col">处理方式</th></tr></thead><tbody>{groups.map(g => <tr key={g.id}><th scope="row"><span className="ds-batch-row">{batch.checkbox(g)}<code>{g.label}</code></span></th><td><span className={g.localExists ? 'ds-presence' : 'ds-absence'}>{g.localExists ? '存在' : '缺失'}</span></td><td><span className={g.remoteExists ? 'ds-presence' : 'ds-absence'}>{g.remoteExists ? '存在' : '缺失'}</span></td><td>{g.differences.length ? <GroupDecision group={g} {...props} preventDelete={preventDelete}/> : <span className="ds-comparison-hint">随记录处理</span>}</td></tr>)}</tbody></table></div></>;
}
...
function DifferenceTable({differences, file, decisions, disabled, onChange}: Props & {differences: DataDifference[]}) {
  const batch = useDataSyncBatch({scope: file.table + ' 逐项处理', groups: differences.map(d => ({id: d.id, label: d.path.join('/'), differences: [d], localExists: d.local !== undefined, remoteExists: d.remote !== undefined})), decisions, disabled, onChange});
  const patch = (d: DataDifference, next: Partial<Decision>) => onChange({...decisions, [d.id]: {...decisionFor(d, decisions), ...next}});
  return <>{batch.tools}<div className="ds-table-scroll"><table className="ds-compare-table ds-detail-table" aria-label={file.table + ' 逐项处理'}><thead><tr><th scope="col">{batch.all}字段</th><th scope="col">GameCreator</th><th scope="col">引擎文件</th><th scope="col">采用值</th></tr></thead><tbody>{differences.map(d => {
    ...
    return <tr key={d.id}><th scope="row">{batch.checkbox({id: d.id, label: d.path.join('/'), differences: [d], localExists: d.local !== undefined, remoteExists: d.remote !== undefined})}<code>{d.path[0] === 'order' ? '记录顺序' : d.path.slice(d.path[0] === 'rows' ? 2 : 1).join(' / ') || d.path[0]}</code>{d.conflict && <small className="ds-pending">双方修改</small>}</th>
