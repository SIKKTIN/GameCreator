...
import {feedbackIntentLabels,feedbackBatchItems,validateFeedbackHistory,type FeedbackReceipt} from '../shared/engine-feedback.mjs';
import {beforeLogoutEvent} from './auth';
import './engine-feedback.css';

type Props={projectId:string;config:EngineConfig;collaboration:CollaborationSource;blockedReason:string;onApplied:(reloadContent?:boolean)=>boolean};
export function EngineFeedbackPanel({projectId,config,collaboration,blockedReason,onApplied}:Props) {
  ...
  const invalidate=()=>{for(const token of tokens.current)void api?.release(token);tokens.current=[];setScan(s=>s?{...s,entries:s.entries.map(e=>({...e,token:undefined}))}:s);setDecisions({});setConfirmed(false);setChangeConfirmed(false);setLegacyConfirmed(false);setBatchConfirmed(false);};
  ...
  const readFeedback=(manual=true)=>run(async()=>{
    ...
    if(result.contentReload){onApplied(true);return;}if(!alive.current||latest.current.source!==request.source||latest.current.blocked){for(const e of result.entries)if(e.token)void api.release(e.token);if(alive.current&&!latest.current.blocked)queued.current=true;return;}
  ...
  const choose=(e:FeedbackEntry)=>{setSelected(e.path);setDecisions({});setConfirmed(false);setChangeConfirmed(false);setLegacyConfirmed(false);setNotice('');};
  ...
  const apply=(dismiss:boolean)=>run(async()=>{
    ...
    try{
      const result=await api.feedbackApply({token:entry.token,decisions,dismiss,acceptCompletion:confirmed,acceptLegacy:legacyConfirmed,acceptProjectChange:changeConfirmed});recordResults([result]);
      if(entry.feedback?.intent==='project_change'&&!dismiss)onApplied(true);if(alive.current)setNotice((dismiss?'反馈已忽略，开发内容保持原样。':'反馈已应用，项目内容已刷新。')+(result.warning?' '+result.warning:''));
  ...
  return <section className="engine-content ef-panel" aria-label="开发反馈">
    ...
    {view==='history'?<div>
      ...
      {history.map(r=><details className="es-history" key={r.id}><summary><span className={'es-state '+(r.outcome==='applied'?'added':'unchanged')}>{r.identity?.intent==='spec_change'?(r.outcome==='applied'?'已采纳':'已拒绝'):(r.outcome==='applied'?'已应用':'已忽略')}</span><b>{r.title}</b><span>{new Date(r.at).toLocaleString()}</span></summary><p>{r.author} · {r.summary}</p>{r.reason&&<p>修改原因：{r.reason} · 影响：{r.impact}</p>}<code>{r.id}</code>{r.compatibility&&<div className="ef-text">{Object.entries({reuse:"复用",modify:"修改",add:"新增",archive:"归档"}).map(([key,label])=><p key={key}>{label}：{r.compatibility![key as keyof typeof r.compatibility]}</p>)}</div>}{r.evidence.map((v,i)=><p className="ef-text" key={i}>{v}</p>)}{r.rows.map(row=><p className="ef-text" key={row.field}>{row.label}：{r.decisions[row.field]==='keep'?'保留 '+(row.current||'空值'):(row.current||'空值')+' → '+(row.incoming||'空值')}</p>)}</details>)}
      ...
      <article className="ef-detail">{entry?.state==='invalid'?<><h3>反馈无法接收</h3><p className="es-error">{entry.error}</p><code>{entry.path}</code><p>按协作说明修正文件后重新读取。已处理的反馈需要使用新的更新编号。</p></>:entry?.feedback&&<>
        <div className="es-section-heading"><div><span className="es-state">{entry.feedback.target.kind==='module'?'项目内容':entry.feedback.target.kind==='task'?'制作任务':'开发工具'}</span><h3>{entry.title}</h3><p>{entry.identity?.memberName||entry.feedback.author} · {entry.feedback.summary}</p></div></div>
        {entry.identity&&<p className="es-success">已验证 AI 身份：{entry.identity.memberName} · {feedbackIntentLabels[entry.identity.intent]}</p>}
        ...
        {['dispatch','spec_change','project_change'].includes(entry.feedback.intent||'')&&<div className="es-notice"><p>{entry.feedback.intent==='dispatch'?'任务派发：确认接收人及职责后更新同一份排期任务。':entry.feedback.intent==='spec_change'?'需求与验收变更建议：提交时保留原需求，确认采纳后才修改任务说明或验收标准。':'正式项目修改：按字段更新项目内容，保留人员授权和历史记录。'}</p><p>修改原因：{entry.feedback.reason}</p><p>影响说明：{entry.feedback.impact}</p><label><input type="checkbox" aria-label="已评估变更内容与影响" checked={changeConfirmed} disabled={busy} onChange={e=>setChangeConfirmed(e.target.checked)}/>已评估变更内容与影响</label></div>}{entry.feedback.intent==='propose'&&<p className="es-notice">本条作为排期与分配建议记录在任务详情，不覆盖制作结果、日期或人员分配。</p>}
        {entry.feedback.compatibility&&<section className="ef-evidence" aria-label="内容更新兼容方案"><h4>内容更新兼容方案</h4>{Object.entries({reuse:"复用",modify:"修改",add:"新增",archive:"归档"}).map(([key,label])=><p className="ef-text" key={key}><b>{label}：</b>{entry.feedback!.compatibility![key as keyof NonNullable<typeof entry.feedback.compatibility>]}</p>)}</section>}
        ...
        <div className="es-footer"><button className="gp-secondary" disabled={busy||!!blocked||!entry.token} onClick={()=>void apply(true)}>{entry.feedback.intent==='spec_change'?'拒绝此变更建议':'忽略本条反馈'}</button><button className="primary" disabled={busy||!!blocked||!entry.token||unresolved||entry.legacy&&!legacyConfirmed||!!completing&&!confirmed||['dispatch','spec_change','project_change'].includes(entry.feedback.intent||'')&&!changeConfirmed} onClick={()=>void apply(false)}><ArrowDownToLine size={16}/>{entry.feedback.intent==='spec_change'?'采纳并修改需求':'应用反馈'}</button></div>
      </>}</article>
    </div>}
  </section>;
}
