...
export type PrototypeExample = {
  schema: 1; name: string; description: string; gameplay: GameplayStore; gameplayCore?: GameplayCoreStore; taskFlows?: TaskFlowStore;
...
const sections = ['task-flows', 'gameplay', 'gameplay-core', 'functional-systems', 'art-assets', 'definitions', 'stories', 'project', 'milestones'] as const;
...
export function validatePrototypeExample(value: unknown): PrototypeExample {
  ...
  requireValid(fields.every(field => Object.prototype.hasOwnProperty.call(value, field)) && Object.keys(value).every(field => [...fields, 'gameplayCore', 'taskFlows'].includes(field)), '包含缺失字段或非便携配置');
...
export function preparePrototypeProject(catalog: ProjectCatalog, value: unknown, name: string): PreparedPrototypeProject {
  ...
  const archives = {
    'task-flows': example.taskFlows ?? emptyTaskFlows(), gameplay: example.gameplay, 'gameplay-core': example.gameplayCore ?? emptyGameplayCore(), 'functional-systems': example.functionalSystems, 'art-assets': example.artAssets,
