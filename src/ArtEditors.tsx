...
import { artCategories, artPriorities, artRequirementStatuses, artReviewStatuses, artSourceText, artRequirementReadiness, canAdoptVersion, type ArtStore, type ArtRequirement, type ArtAsset, type ArtVersion, type ArtFile, type ArtSource, type ArtSources } from './art-assets';
...
export function RequirementEditor({ requirement: r, controller, sources, apply, onChange, onOpenAsset, onOpenGameplay, onOpenCapability, unified=false }: { unified?:boolean; requirement: ArtRequirement; controller: ArtController; sources: ArtSources; apply: Change; onChange: (changes: Partial<ArtRequirement>) => void; onOpenAsset: (id: string) => void; onOpenGameplay: Navigation['onOpenGameplay']; onOpenCapability: Navigation['onOpenCapability'] }) {
  ...
  return <><div className="gp-editor-heading"><div><span className="gp-kicker">ART REQUIREMENT · {r.category}</span><h2>{r.name || '未命名需求'}</h2><p className="gp-muted">明确交付要求，关联实际使用场景。</p></div><button className="gp-secondary" disabled={controller.blocked} onClick={() => onChange({ archived: !r.archived })}>{r.archived ? <RotateCcw size={14} /> : <Archive size={14} />}{r.archived ? '恢复素材需求' : '归档素材需求'}</button></div>
    ...
    <div className={'ar-readiness' + (r.status === '已通过' && readiness.ready ? ' complete' : '')}><div><Check size={18} /><strong>{r.status === '已通过' && readiness.ready ? '正式需求已验收' : readiness.ready ? '正式资产已就绪，等待需求验收' : '正式交付尚未完成'}</strong></div><p>{readiness.ready ? '关联资产均已采用审核通过的正式版本，需求状态由你确认。' : readiness.issues.join('；')}</p></div>
    <section className="gp-card"><div className="gp-card-heading"><h3>需求说明</h3><span className="gp-muted">占位资产不计入正式验收</span></div><fieldset className="ar-fields" disabled={disabled}>
      ...
      <label className="gp-field">制作状态<select aria-label="需求状态" value={r.status} onChange={e => onChange({ status: e.target.value as ArtRequirement['status'] })}>{artRequirementStatuses.map(s => <option key={s} value={s} disabled={s === '已通过' && !readiness.ready}>{s==='已通过'?'已完成':s}</option>)}</select></label>
      {r.status === '已通过' && <p className="gp-muted">更换或移除当前交付前，请先将需求退回制作或审核状态。</p>}
...
export function RequirementAssets({ requirement, store, disabled, apply, onOpenAsset }: { requirement: ArtRequirement; store: ArtStore; disabled: boolean; apply: Change; onOpenAsset: (id: string) => void }) {
  ...
  return <section className="gp-card ar-linked-assets"><div className="gp-card-heading"><div><h3>关联交付资产</h3><p className="gp-muted">一份需求可以使用多个资产，同一资产也可以复用于多个需求。</p></div><ImageIcon size={18} /></div>
    {links.length ? <div className="ar-reference-list">{links.map(link => { const asset = store.assets.find(a => a.id === link.assetId), version = asset?.versions.find(v => v.id === asset.adoptedVersionId); return <div className="ar-reference" key={link.id}><div className="ar-reference-heading"><button className="gp-link-name" disabled={!asset} onClick={() => onOpenAsset(link.assetId)}>{asset?.name || '资产已失效'}<ArrowRight size={13} /></button><button className="gp-icon" aria-label={'解除素材资产关联：' + link.id} disabled={disabled} onClick={() => apply(current => ({ ...current, links: current.links.filter(l => l.id !== link.id) }))}><Trash2 size={14} /></button></div><div className="ar-asset-link-status"><span className={'ar-delivery-badge' + (version && !version.placeholder && version.review === '已通过' ? ' ready' : '')}>{asset ? adoptedAssetText(asset) : '请解除失效关联'}</span>{asset?.archived && <span className="ar-warning">资产已归档</span>}{version && <small>{version.files.length} 个文件 · {version.review}</small>}</div><TextField label="使用说明" ariaLabel={'素材资产使用说明：' + link.id} rows={2} value={link.note} disabled={disabled} onChange={note => apply(current => ({ ...current, links: current.links.map(l => l.id === link.id ? { ...l, note } : l) }))} /></div>; })}</div> : <Empty>尚未关联资产。可先在资产库创建并导入交付版本。</Empty>}
...
export function ArtReferences({ controller, sources, kind, targetId, onOpenRequirement }: { controller: ArtController; sources: ArtSources; kind: 'gameplay' | 'capability'; targetId: string; onOpenRequirement: (id: string) => void }) {
  ...
  return <section className="gp-card ar-backlinks" aria-label="关联素材需求"><div className="gp-card-heading"><div><h3>素材需求</h3><p className="gp-muted">在素材资产模块关联需求，这里会同步显示。</p></div><Palette size={18} /></div>{controller.blocked && <Notice>素材存档无法读取，关联内容暂时不可用。</Notice>}{matches.length ? <div className="ar-reference-list">{matches.map(({ requirement: r, links }) => <div className="ar-reference" key={r.id}><div className="ar-reference-heading"><button className="gp-link-name" aria-label={'查看素材需求：' + r.name} onClick={() => onOpenRequirement(r.id)}>{r.name}<ArrowRight size={13} /></button><span className="gp-badge">{r.archived ? '已归档' : r.status}</span></div>{links.map(link => <div key={link.id} className="ar-backlink-origin"><small>{artSourceText(link, sources)}</small>{link.note && <p>{link.note}</p>}</div>)}</div>)}</div> : <Empty>尚无素材需求引用此{kind === 'gameplay' ? '玩法' : '功能'}。</Empty>}</section>;
}
export function AssetEditor({ asset, controller, apply, onChange, onOpenRequirement, importRequest=0, onBusy, unified=false }: { importRequest?:number; onBusy?:(busy:boolean)=>void; unified?:boolean; asset: ArtAsset; controller: ArtController; apply: Change; onChange: (changes: Partial<ArtAsset>) => void; onOpenRequirement: (id: string) => void }) {
  ...
  useEffect(()=>{if(importRequest)void importFiles();},[importRequest]);
