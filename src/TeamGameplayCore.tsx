...
export function TeamGameplayCore({ session, projectId, blocked, onDenied }: { session: TeamSession; projectId: string; blocked: boolean; onDenied: (status?: number) => void }) {
  ...
  const move = (updates: CorePosition[]) => {
    if (initialLayout.error) return;
    const validIds = new Set(current.current.store.graphs.flatMap(g => g.nodes.map(n => n.id)));
    const next = { ...positions.current };
    for (const { id, x, y } of updates) if (validIds.has(id) && Number.isFinite(x) && Number.isFinite(y)) next[id] = { x, y };
    positions.current = next; setLayout(next);
  ...
  return <section className="team-core" aria-label="团队玩法核心">
    ...
    {(error || diskError || layoutError || syncError) && <div className="team-message" role="alert">{error || diskError || layoutError || syncError}{diskError && !initial.error && <button onClick={() => persist(current.current)}>重试保存草稿</button>}{layoutError && !initialLayout.error && <button onClick={() => { move([]); }}>重试保存个人布局</button>}</div>}
