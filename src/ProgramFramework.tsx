...
export function ProgramFramework({controller,engine,onOpenEngine}:{controller:ProgramFrameworkController;engine:string;onOpenEngine:()=>void}) {
  ...
  return <section className="pf-page" aria-label="程序框架工作区">
    ...
    {tab==='library'?<div className="pf-library">
      <aside className="pf-catalog" aria-label="框架规范目录"><div className="pf-search"><Search size={17}/><input aria-label="搜索框架规范" placeholder="搜索标题或正文…" value={query} onChange={e=>{leaveSearch();setQuery(e.target.value);}}/></div><p className="pf-muted">{filtered.length} 份文档{query?'匹配当前搜索':''}</p><div className="pf-doc-list">{[...new Set(filtered.map(d=>d.group))].map(group=><div key={group}><h3>{group}</h3>{filtered.filter(d=>d.group===group).map(d=><button key={d.id} className={selected===d.id?'selected':''} aria-label={'阅读框架规范：'+d.title} aria-current={selected===d.id?'page':undefined} onClick={()=>{leaveSearch();setSelected(d.id);}}><span>{d.title.replace(/^可选扩展：/,'')}</span>{(d.id==='config-data-policy'||adopted.some(a=>a.id===d.id))&&<Check size={14} aria-label={d.id==='config-data-policy'?'默认加入':'本项目已采用'}><title>{d.id==='config-data-policy'?'默认加入':'本项目已采用'}</title></Check>}</button>)}</div>)}{!filtered.length&&<p className="pf-empty">没有找到匹配文档，试试其他关键词。</p>}</div></aside>
      ...
      {notice&&<p className="pf-notice" role="status">{notice}</p>}<div className="pf-reader" ref={reader}><FrameworkMarkdown doc={doc} onOpen={open}/></div><footer className="pf-document-footer"><span>{doc.id==='config-data-policy'?'默认加入 · 独立于框架采用，随项目开发文档交付':adopted.some(d=>d.id===doc.id)?'已纳入本项目 AI 文档':doc.kind==='reference'?'阅读参考，完整内容可从规范包获取':'尚未纳入本项目 AI 文档'}</span><button onClick={()=>{leaveSearch();setTab('project');}}>管理采用方案 →</button></footer></div>
      ...
      <aside className="pf-export-plan"><h3>本项目导出范围</h3><p>{store.enabled?`${adopted.length} 份规范与项目约定会进入“程序框架”模块文档。`:'尚未采用内置规范，AI 文档只记录采用状态与项目约定。'}</p>{adopted.map(d=><button key={d.id} onClick={()=>open(d.id)}><Check size={14}/>{d.title}</button>)}<p className="pf-muted">总文档、模块文档与引擎文档同步共用这份采用方案。配置数据管理与同步规范默认随项目开发文档交付。全部 {frameworkLibrary.documents.length} 份文档仍可在规范库阅读与下载。</p></aside>
    </div>}
  </section>;
}
