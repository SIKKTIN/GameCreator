...
function ContentSync({projectId,config,build,art,collaboration,blockedReason,onOpenAsset,tab,setTab}:Props&{tab:string;setTab:(tab:string)=>void}) {
  ...
  return <div className="engine-content" hidden={tab==='connection'||tab==='feedback'}>
    ...
    {tab==='settings'&&<>
      ...
      <section className="es-card es-collaboration"><label className="es-checkbox"><input type="checkbox" checked={settings.collaboration} disabled={busy} onChange={e=>patch({collaboration:e.target.checked})}/>同步开发协作（任务与工具回写）</label><p>在工程的 gamecreator/ 中生成必读的项目规范、任务与工具上下文、反馈填写规范和比较基准。开发者或 AI 提交反馈后，到“开发反馈”查看并应用。</p><div className="es-target-path"><span>开发协作目录</span><output>{fullPath('gamecreator')}</output></div><small>关闭后保留已有上下文、反馈及回执。既有同步配置需要手动开启；历史快照保留用于核对旧反馈。</small></section>
