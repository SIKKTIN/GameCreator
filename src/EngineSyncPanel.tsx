...
import type {SyncHistory,SyncPlan,SyncBinding,CollaborationSource} from './engine-sync';
...
function ContentSync({projectId,config,build,art,collaboration,blockedReason,onOpenAsset,tab,setTab}:Props&{tab:string;setTab:(tab:string)=>void}) {
  ...
  let source='';try{const doc=build();source=JSON.stringify({art,collaboration,sections:doc.sections,name:doc.projectName,version:doc.version});}catch{/* blocked above */}
  ...
  return <div className="engine-content" hidden={tab==='connection'||tab==='feedback'}>
    ...
    {(tab==='settings'||tab==='preview')&&settings.assets&&unadopted.length>0&&<section className="es-notice es-material-pending" aria-label="未纳入同步的素材">
      <strong>{unadopted.length} 项历史资源没有采用记录，仅保留在项目存档中</strong>
      <p>新素材请按制作文档直接生成到工程，通过开发反馈更新进度。无需重新上传或采用这些历史文件。</p>
      <ul>{unadopted.map(asset=><li key={asset.id}><div><b>{asset.name}</b><small>历史文件保留</small></div>{onOpenAsset&&<button className="gp-secondary" disabled={busy} onClick={()=>onOpenAsset(asset.id)}>查看制作文档</button>}</li>)}</ul>
    </section>}
    {tab==='settings'&&<>
      ...
      <fieldset disabled={busy} className="es-fields">
        <div className="es-config-grid">
          <section className="es-card"><label className="es-checkbox"><input type="checkbox" checked={settings.documents} onChange={e=>patch({documents:e.target.checked})}/>同步设计文档</label><p>按模块生成 Markdown，可供程序与 AI 查阅。同时附带配置数据管理与同步规范，无需采用程序框架。</p><label>文档子目录（相对于工程根目录）<input aria-label="文档目标目录" value={settings.docsDirectory} onChange={e=>patch({docsDirectory:e.target.value})}/></label><div className="es-target-path"><span>最终写入位置</span><output aria-label="文档最终写入位置">{fullPath(settings.docsDirectory)}</output></div><div className="es-modules">{aiModules.filter(m=>available.includes(m.id)).map(m=><label className="es-checkbox" key={m.id}><input type="checkbox" disabled={!settings.documents} checked={settings.modules.includes(m.id)} onChange={e=>patch({modules:e.target.checked?[...settings.modules,m.id]:settings.modules.filter(id=>id!==m.id)})}/>{m.label}</label>)}</div><small>{config.engine==='godot-gdscript'?'文档以 Markdown 同步，可在 Godot 文件系统中查看。若旧版留下 .gdignore，请在待同步变更中确认移除。':'文档用于开发查阅，发布游戏时请按工程规则排除。'}</small></section>
