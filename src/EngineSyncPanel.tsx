...
import {defaultSyncSettings,syncSettings,type SyncSettings} from '../shared/engine-sync.mjs';
...
export function EngineSyncPanel(props:Props) {
  ...
  return <section className="engine-hub" aria-label="引擎与同步">
    ...
    <div hidden={tab!=='connection'}><EngineSettings config={props.config} setConfig={props.setConfig} registry={props.registry} onPickDirectory={props.onPickDirectory}/></div>
...
function ContentSync({projectId,config,build,art,blockedReason,tab,setTab}:Props&{tab:string;setTab:(tab:string)=>void}) {
  ...
  return <div className="engine-content" hidden={tab==='connection'}>
    ...
    {tab==='settings'&&<>
      <div className="es-section-heading"><div><h3>同步范围与目录</h3><p>目录相对于当前工程。关闭某类同步会保留其已交付文件。</p></div><button className="primary" disabled={busy||!dirty||!!invalid} onClick={save}><Save size={16}/>保存同步配置</button></div>
      <fieldset disabled={busy} className="es-fields">
        <div className="es-config-grid">
          <section className="es-card"><label className="es-checkbox"><input type="checkbox" checked={settings.documents} onChange={e=>patch({documents:e.target.checked})}/>同步设计文档</label><p>按模块生成 Markdown，可供程序与 AI 查阅。</p><label>文档目标目录<input aria-label="文档目标目录" value={settings.docsDirectory} onChange={e=>patch({docsDirectory:e.target.value})}/></label><div className="es-modules">{aiModules.filter(m=>available.includes(m.id)).map(m=><label className="es-checkbox" key={m.id}><input type="checkbox" disabled={!settings.documents} checked={settings.modules.includes(m.id)} onChange={e=>patch({modules:e.target.checked?[...settings.modules,m.id]:settings.modules.filter(id=>id!==m.id)})}/>{m.label}</label>)}</div><small>{config.engine==='godot-gdscript'?'文档目录包含 .gdignore，用于开发查阅，不参与游戏资源导入。':'文档用于开发查阅，发布游戏时请按工程规则排除。'}</small></section>
          <section className="es-card"><label className="es-checkbox"><input type="checkbox" checked={settings.assets} onChange={e=>patch({assets:e.target.checked})}/>同步已采用素材</label><p>交付采用版本的原文件，历史版本仍保留在素材资产中。</p><label>素材目标目录<input aria-label="素材目标目录" value={settings.assetsDirectory} onChange={e=>patch({assetsDirectory:e.target.value})}/></label><label className="es-checkbox"><input type="checkbox" disabled={!settings.assets} checked={settings.includePlaceholders} onChange={e=>patch({includePlaceholders:e.target.checked})}/>包含已采用的占位素材</label><small>关闭后，仅纳入审核通过的正式采用版本。单文件素材更新时保留同格式的工程路径；多文件版本按文件名对应。</small><div className="es-policy"><b>交付规则</b><p>更新和移除前自动备份。工程中手工修改过的文件需处理冲突。不会修改程序、场景或引擎生成的导入缓存。</p></div></section>
        </div>
      </fieldset>
