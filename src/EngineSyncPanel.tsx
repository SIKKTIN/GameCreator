    ...
    {tab==='settings'&&<>
      ...
      <fieldset disabled={busy} className="es-fields">
        <div className="es-config-grid">
          <section className="es-card"><label className="es-checkbox"><input type="checkbox" checked={settings.documents} onChange={e=>patch({documents:e.target.checked})}/>同步设计文档</label><p>按模块生成 Markdown，可供程序与 AI 查阅。</p><label>文档子目录（相对于工程根目录）<input aria-label="文档目标目录" value={settings.docsDirectory} onChange={e=>patch({docsDirectory:e.target.value})}/></label><div className="es-target-path"><span>最终写入位置</span><output aria-label="文档最终写入位置">{fullPath(settings.docsDirectory)}</output></div><div className="es-modules">{aiModules.filter(m=>available.includes(m.id)).map(m=><label className="es-checkbox" key={m.id}><input type="checkbox" disabled={!settings.documents} checked={settings.modules.includes(m.id)} onChange={e=>patch({modules:e.target.checked?[...settings.modules,m.id]:settings.modules.filter(id=>id!==m.id)})}/>{m.label}</label>)}</div><small>{config.engine==='godot-gdscript'?'文档目录包含 .gdignore，用于开发查阅，不参与游戏资源导入。':'文档用于开发查阅，发布游戏时请按工程规则排除。'}</small></section>
