...
function UserPermissionsContent({session,onConnect,onChanged}:{session:TeamSession;onConnect:()=>void;onChanged:()=>void}) {
  ...
  return <>
    ...
    {!denied&&loaded&&<>
      ...
      <section className="overview-panel" aria-label="项目权限管理"><h2>{deletionEnabled?'协作项目管理':'我管理的协作项目'}</h2><p>编辑者默认概览和排期只读，故事、玩法核心和玩法设计可编辑。项目管理员配置成员权限；服务器管理员可删除此服务器的协作项目。</p>
