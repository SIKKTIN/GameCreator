...
import { prototypeSource } from './prototype-design';
...
function WorkspaceController({ role, username }: { role: UserRole; username: string }) {
  ...
  const transfer = useProjectTransfer({ projects, lock, allowed: role === 'admin', allowSwitch,
    onImported: () => { setActiveTeamId(null); if (storedTest) setStoredTest(null); setError(''); } });
  ...
  const publishProject = () => {
    if (!formalProject || role !== 'admin' || testSession || activeTeamId || lock.current || projects.blocked || !allowSwitch()) return;
  ...
  const configureProject = async (config: EngineConfig) => {
    if (!formalProject || role !== 'admin' || lock.current || projects.blocked) return false;
  ...
  const load = async (scenario: TestScenarioId) => {
    if (!formalProject || role !== 'admin' || lock.current || projects.blocked) return;
    lock.current = true; setPreparing(true); setError('');
    try {
  ...
  return <><TeamConnectionDialog connection={team} onConnected={id => {
    ...
    {selectedTeam && team.session ? <TeamProjectWorkspace key={`${team.session.serverId}:${team.session.user.id}:${selectedTeam.id}:${team.session.token}`} project={selectedTeam} session={team.session}
      ...
      picker={<ProjectSwitcher projects={options} teamNotice={teamNotice} currentId={teamProjectKey(team.session.serverId, selectedTeam.id)} currentName={selectedTeam.name} canAdd={role === 'admin'} busy={preparing || prototypeBusy || transfer.busy || projects.blocked}
        onSelect={selectProject} onAdd={addProject} onDelete={deleteProject} onConnectTeam={connectTeam} onCreateTeam={createTeam} onImportPrototype={openPrototypeImport} onImportProject={transfer.enabled ? transfer.openImport : undefined} />}
      onConnection={connectTeam} onDisconnect={() => { if (allowSwitch()) { team.disconnect(); setActiveTeamId(null); } }} />
    : formalProject ? <ProjectDataUpgrade key={testSession?.id ?? 'project:' + formalProject.id} role={role} username={username}
    {...serverNavigation}
    ...
    : <div className="app local-workspace empty-project-workspace">
      <WorkspaceSidebar active={serverNavigation.serverPage ? serverNavigation.adminPageName??'服务器管理' : ''} onNavigate={() => {if(canLeaveTeam())leaveServer();}} admin={role === 'admin'} onManageServer={() => openServer()} onManageUsers={serverNavigation.onManageUsers} empty
        picker={<ProjectSwitcher projects={options} teamNotice={teamNotice} currentId={null} currentName="未选择项目" canAdd={role === 'admin'} busy={prototypeBusy || transfer.busy}
          onSelect={selectProject} onAdd={addProject} onDelete={deleteProject} onConnectTeam={connectTeam} onCreateTeam={createTeam} onImportPrototype={openPrototypeImport} onImportProject={transfer.enabled ? transfer.openImport : undefined} />}
      ...
      {serverNavigation.serverPage || <main className="empty-project-main"><FolderOpen size={40} /><h1>暂无本地项目</h1>
        <p>{role === 'admin' ? '从左上角项目菜单新建项目、导入原型示例或项目文件夹。' : '请联系管理员创建或导入本地项目。'}</p>
...
function ProjectDataUpgrade(props: ComponentProps<typeof WorkspaceApp>) {
  ...
  if (error) return <main className="enum-catalog-empty" role="alert">
    ...
    <ProjectSwitcher projects={props.projectOptions} currentId={props.formalProject.id} currentName={props.formalProject.name}
      canAdd={props.role === 'admin'} busy={props.preparingTest} onSelect={props.onSelectProject} onAdd={props.onAddProject} onDelete={props.onDeleteProject} />
  </main>;
...
function WorkspaceApp({ role, username, testSession, onLoadTest, onExitTest, preparingTest, testError, formalProject, projectOptions, onSelectProject, onAddProject, onDeleteProject, onConfigChange, onRenameProject, onConnectTeam, onCreateTeam, onPublishProject, teamNotice, onImportPrototype, onImportProject, onExportProject, serverPage, onManageServer, onLeaveServer, adminPageName, onManageUsers }: {
  ...
  useEffect(() => {
    const guard = (event: Event) => { if (gameplay.pending || functional.pending || art.pending || core.pending || prototype.pending || tasks.pending || narrative.pending) event.preventDefault(); };
    window.addEventListener(beforeLogoutEvent, guard);
    return () => window.removeEventListener(beforeLogoutEvent, guard);
  }, [gameplay.pending, functional.pending, art.pending, core.pending, prototype.pending, tasks.pending, narrative.pending]);
  ...
  const storageError = [definitionsError, milestoneError, storyError, projectError, registry.error, gameplay.error, functional.error, art.error, core.error, prototype.error, tasks.error, narrative.error].filter(Boolean).join('；');

  const exportAiContext = async () => {
    if (testSession || gameplay.blocked || gameplay.pending || functional.blocked || functional.pending || art.blocked || art.pending || core.blocked || prototype.blocked || tasks.blocked || narrative.blocked || core.pending || prototype.pending || tasks.pending || narrative.pending) return;
    const markdown = buildAiMarkdown(project, storyDocs, currentData, definitions, engineConfig, registry, gameplay.store.designs, functional.store, art.store, core.store, prototype.store, tasks.store, narrative.store);
    ...
    <div className="app local-workspace">
      ...
      <main hidden={!!serverPage} className={active === '玩法核心' ? 'core-workspace-page' : undefined}>
        <header>
          ...
          <div className="header-actions">
            {role === 'admin' && <TestPanel page={active} username={username} config={engineConfig} registry={registry} testSession={testSession}
              busy={preparingTest || gameplay.pending || functional.pending || functional.blocked || art.pending || art.blocked || core.pending || prototype.pending || tasks.pending || narrative.pending} error={testError || storageError} onLoad={onLoadTest} onExit={onExitTest} onNavigate={setActive} />}
            <div className="search"><Search size={16} /><input placeholder="搜索内容..." /></div>
            <button className="save" disabled={!!testSession || gameplay.blocked || gameplay.pending || functional.blocked || functional.pending || art.blocked || art.pending || core.blocked || prototype.blocked || tasks.blocked || narrative.blocked || core.pending || prototype.pending || tasks.pending || narrative.pending} title={testSession ? "测试工作区可在面板复制诊断信息" : undefined} onClick={exportAiContext}><FileText size={16} />生成 AI 文档</button>
        ...
        {testSession && <div className="test-workspace-banner" role="status"><span><b>测试工作区</b> · {testScenarios.find(item=>item.id===testSession.scenario)?.name} · 数据独立保存</span>
          <button disabled={preparingTest || registry.busy || registry.loading || gameplay.pending || functional.pending || functional.blocked || art.pending || art.blocked || core.pending || prototype.pending || tasks.pending || narrative.pending} onClick={onExitTest}>返回原工作区</button></div>}
        {testError && <p className="field-error" role="alert">{testError}</p>}
        {storageError && !gameplay.error && !functional.error && !art.error && !core.error && !prototype.error && !tasks.error && !narrative.error && <p className="field-error" role="alert">{storageError}</p>}
        ...
        {active === '原型设计' && <PrototypeDesign controller={prototype} designs={gameplay.store.designs} core={core.store} art={art.store} workspaceId={art.workspaceId} onOpenGameplay={openGameplay} />}
        ...
        {active === '工作区设置' && <StoryModuleSettings controller={narrative} onOpen={()=>setActive('故事编排')}/>}
        {active === '故事编排' && narrative.store.enabled && <StoryOrchestration art={art.store} workspaceId={art.workspaceId} controller={narrative} data={currentData} tasks={tasks.store.tasks} selectedId={narrativeId} onSelect={setNarrativeId} onOpenTask={id=>{setRequestedTask({id});setActive('任务与流程');}}/>}
        ...
        {active !== '工作区设置' && active !== '故事编排' && active !== '原型设计' && active !== '任务与流程' && active !== '项目概览' && active !== '玩法核心' && active !== '玩法设计' && active !== '功能系统' && active !== '美术资产' && active !== '故事文档' && active !== '数据配置' && active !== '枚举定义' && active !== '枚举管理' && active !== '引擎设置' && (
