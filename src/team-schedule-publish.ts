...
import {withScheduleReleases} from './schedule-releases.ts';
...
export function readLocalSchedule(storage:Pick<Storage,'getItem'>,project:SavedProject) {
  ...
  const schedule=normalizeSchedulePublication({store:withScheduleReleases(store),references});return {schedule,signature:JSON.stringify(schedule)};
}
