...
export type ProjectPackageDocument = {
  ...
  archives: {
    gameplay: GameplayStore; 'gameplay-core': GameplayCoreStore; 'prototype-design': PrototypeDesignStore; 'functional-systems': FunctionalStore; 'art-assets': ArtStore;
...
export const projectPackageSections = ['gameplay', 'gameplay-core', 'prototype-design', 'functional-systems', 'art-assets', 'definitions', 'stories', 'project', 'milestones', 'enum-versions'] as const;
...
export function validateProjectPackage(value: unknown): ProjectPackageDocument {
  ...
  requireValid(projectPackageSections.filter(section => section !== 'gameplay-core' && section !== 'prototype-design').every(section => Object.prototype.hasOwnProperty.call(archives, section)) && Object.keys(archives).every(key => [...projectPackageSections, 'data-view'].includes(key as typeof projectPackageSections[number])), '项目模块缺失或版本不受支持');
...
export function captureProjectPackage(storage: Pick<Storage, 'getItem'>, project: SavedProject): ProjectPackageSnapshot {
  ...
  const archives = {
    gameplay: read('gameplay', emptyGameplay()), 'gameplay-core': read('gameplay-core', emptyGameplayCore()), 'prototype-design': read('prototype-design', emptyPrototypeDesign()), 'functional-systems': read('functional-systems', emptyFunctionalSystems()),
