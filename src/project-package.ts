...
export type ProjectPackageDocument = {
  ...
  archives: {
    'task-flows': TaskFlowStore; gameplay: GameplayStore; 'gameplay-core': GameplayCoreStore; 'functional-systems': FunctionalStore; 'art-assets': ArtStore;
...
export const projectPackageSections = ['task-flows', 'gameplay', 'gameplay-core', 'functional-systems', 'art-assets', 'definitions', 'stories', 'project', 'milestones', 'enum-versions'] as const;
...
export function validateProjectPackage(value: unknown): ProjectPackageDocument {
  ...
  requireValid(projectPackageSections.filter(section => section !== 'gameplay-core' && section !== 'task-flows').every(section => Object.prototype.hasOwnProperty.call(archives, section)) && Object.keys(archives).every(key => [...projectPackageSections, 'data-view'].includes(key as typeof projectPackageSections[number])), '项目模块缺失或版本不受支持');
...
export function captureProjectPackage(storage: Pick<Storage, 'getItem'>, project: SavedProject): ProjectPackageSnapshot {
  ...
  const archives = {
    'task-flows': read('task-flows', emptyTaskFlows()), gameplay: read('gameplay', emptyGameplay()), 'gameplay-core': read('gameplay-core', emptyGameplayCore()), 'functional-systems': read('functional-systems', emptyFunctionalSystems()),
