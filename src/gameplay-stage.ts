...
export type StageObject = { id: string; name: string; kind: keyof typeof stageKinds; color: keyof typeof stageColors; anchor: 'cell' | 'left' | 'right'; row: number; column: number; width: number; height: number; direction: 'left' | 'right' | 'up' | 'down'; rangeShape: 'none' | 'line' | 'radius'; range: number; notes: string };
export type StageLayout = { rows: number; columns: number; cellSize: number; unit: string; description: string; objects: StageObject[] };
...
export function validateStage(value: GameplayStage) {
  ...
  if (!object(s) || !num(s.rows, 1, 30, true) || !num(s.columns, 1, 40, true) || !num(s.cellSize, .001, 10000) || !strings(s, ['unit', 'description']) ||
      !list(s.objects, o => strings(o, ['name', 'notes']) && has(stageKinds, o.kind) && has(stageColors, o.color) && ['cell', 'left', 'right'].includes(o.anchor as string) && ['left', 'right', 'up', 'down'].includes(o.direction as string) && ['none', 'line', 'radius'].includes(o.rangeShape as string) && num(o.row, 1, 30, true) && num(o.column, 1, 40, true) && num(o.width, 1, 40, true) && num(o.height, 1, 30, true) && num(o.range, 0, 100), 300) ||
...
export function copyStage(source: GameplayDesign, newDesignId: string): GameplayStage {
  ...
  return { space: { ...source.space, objects: source.space.objects.map(o => ({ ...o, id: objectIds.get(o.id)! })) }, timeline: { ...source.timeline,
...
export function objectFits(o: StageObject, s: StageLayout) { return o.row + o.height - 1 <= s.rows && (o.anchor === 'cell' ? o.column + o.width - 1 <= s.columns : o.width === 1 && o.height === 1); }
...
export function stageIssues(design: GameplayDesign, designs: GameplayDesign[]) {
  const issues: string[] = []; const s = design.space, t = design.timeline, owner = spaceOwner(design, designs);
...
export function stageMarkdown(d: GameplayDesign, designs: GameplayDesign[]) {
  const s = d.space, t = d.timeline, owner = spaceOwner(d, designs), lines = ['#### 空间布局', '', `${s.rows} 行 × ${s.columns} 列；每格 ${s.cellSize} ${s.unit || '单位'}。行从上到下，列从左到右。`, s.description, ''];
  if (!s.objects.length) lines.push('暂无空间对象。');
  for (const o of s.objects) lines.push(`- ${o.name || '未命名'}（${stageKinds[o.kind]}）：${o.anchor === 'cell' ? `R${o.row} C${o.column}，占 ${o.width}×${o.height} 格` : `第 ${o.row} 行${o.anchor === 'left' ? '左' : '右'}侧边界外`}；朝向 ${o.direction}；范围 ${o.rangeShape === 'none' ? '无' : (o.rangeShape === 'line' ? '直线 ' : '圆形半径 ') + o.range + ' 格'}。${o.notes}`);
