...
export function PrototypeCanvas({ scene, designs, maps, art, workspaceId, selected = '', onSelect, onMove, runtime, onActivate, disabled }: { maps?: MapDesignStore; scene: PrototypeScene; designs: GameplayDesign[]; art: ArtStore; workspaceId: string; selected?: string; onSelect?: (id: string) => void; onMove?: (id: string, x: number, y: number) => void; runtime?: PrototypeRuntime; onActivate?: (id: string) => void; disabled?: boolean }) {
  ...
  return <div className="pd-frame-wrap"><svg ref={svg} className="pd-frame" role="group" aria-label={runtime ? '原型运行画面' : '原型编辑画布'} viewBox={`0 0 ${scene.width} ${scene.height}`} style={{ aspectRatio: `${scene.width}/${scene.height}`, background: scene.background }} onContextMenu={e => e.preventDefault()} onPointerDown={e => begin(e)} onPointerMove={e => { const g = gesture.current; if (!g || g.pointer !== e.pointerId) return; const p = position(e); if (Math.abs(p.x - g.original.x) + Math.abs(p.y - g.original.y) > 2) g.moved = true; if (g.kind === 'pan') setCamera(c => ({ ...c, ...p })); else if (g.moved) setDraft({ id: g.id, ...p }); }} onPointerUp={finish} onPointerCancel={cancel} onLostPointerCapture={cancel}>
    <g ref={world} transform={runtime ? undefined : `translate(${camera.x} ${camera.y}) scale(${camera.zoom})`}>
      ...
      {geometry.objects.map(g => {
        const zone = g.object.kind === 'zone', ring = zone && g.object.rangeShape === 'ring';
        const color = stageColors[g.object.color], cx = g.width / 2, cy = g.height / 2, r = g.rangePixels, inner = g.innerRangePixels;
        return <g key={g.object.id} data-source-object={g.object.id} transform={`translate(${g.x} ${g.y}) rotate(${g.rotation} ${cx} ${cy})`} pointerEvents="none"><title>{g.object.name}</title>
          {ring ? <g data-range="ring" transform={`translate(${cx} ${cy})`}><path fillRule="evenodd" d={`M ${r} 0 A ${r} ${r} 0 1 0 ${-r} 0 A ${r} ${r} 0 1 0 ${r} 0 Z M ${inner} 0 A ${inner} ${inner} 0 1 0 ${-inner} 0 A ${inner} ${inner} 0 1 0 ${inner} 0 Z`} fill={color + '16'} stroke={color} strokeDasharray="6 5" /></g> : g.shape === 'circle' ? <ellipse cx={cx} cy={cy} rx={cx} ry={cy} fill={color + '50'} stroke={color} /> : <rect width={g.width} height={g.height} rx="3" fill={color + '50'} stroke={color} />}
          <text transform={`rotate(${-g.rotation} ${cx} ${cy})`} x={zone ? ring ? cx - r + 6 : 6 : cx} y={zone ? ring ? cy - r + 18 : 18 : cy} textAnchor={zone ? 'start' : 'middle'} dominantBaseline="middle" fill="#f5f3ef" fontSize={zone ? 13 : Math.max(7, Math.min(15, g.width / Math.max(2, g.object.name.length)))}>{g.object.name}</text>
        </g>;
      })}
      {scene.elements.map(e => { const visible = runtime ? prototypeVisible(runtime, e) : true; if (!visible) return null;
        ...
        return <g key={e.id} data-element={e.id} role="button" tabIndex={runtime && !active ? -1 : 0} aria-label={(runtime ? '交互：' : '选择元素：') + e.name} transform={`translate(${g.x} ${g.y}) rotate(${rotation} ${g.width / 2} ${g.height / 2})`} opacity={!runtime && !e.visible ? .4 : 1} style={{ cursor: active ? 'pointer' : runtime ? 'default' : e.sourceObjectId ? 'pointer' : 'move' }} onPointerDown={ev => { if (!runtime) { ev.stopPropagation(); begin(ev, e); } }} onClick={() => { if (runtime && active) onActivate?.(e.id); }} onKeyDown={ev => { if (['Enter', ' '].includes(ev.key)) { ev.preventDefault(); if (runtime) { if (active) onActivate?.(e.id); } else onSelect?.(e.id); } }}>
          ...
          {e.kind === 'image' ? <AssetImage file={file} workspaceId={workspaceId} width={g.width} height={g.height} /> : e.kind !== 'shape' && (e.kind !== 'hotspot' || !runtime) && <text transform={`rotate(${-rotation} ${g.width / 2} ${g.height / 2})`} x={g.width / 2} y={g.height / 2} textAnchor="middle" dominantBaseline="middle" fill={e.kind === 'text' ? e.color : '#ffffff'} fontSize={Math.max(1, Math.min(e.fontSize, g.height * .65, (g.width - 8) / Math.max(1, Array.from(e.text).reduce((n, ch) => n + (ch.codePointAt(0)! > 255 ? 1 : .6), 0))))} pointerEvents="none">{e.text}</text>}
