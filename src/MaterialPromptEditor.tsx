...
export function MaterialPromptEditor({requirement,categoryName,disabled,onChange}:{requirement:ArtRequirement;categoryName:string;disabled:boolean;onChange:(value:MaterialGenerationPrompt)=>void}) {
  ...
  const copy=async()=>{setCopyError('');setNotice('');try{await navigator.clipboard.writeText(materialPromptText(value));setNotice('提示词已复制');}catch{setCopyError('复制失败，请选中下方文本手动复制。');}};
