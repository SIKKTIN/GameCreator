...
GameCreator 是一个面向游戏内容和数据配置的本地客户端。当前包含项目切换、项目概览、玩法核心、玩法设计、功能系统、美术资产、故事文档、数据配置、Lua 枚举扫描、稳定版本审核，以及管理员/用户登录。
...
同一台服务器可保存多个协作项目。管理员从项目列表选择“新建协作项目”，使用团队 admin 连接后填写名称、配置成员权限，即可进入空白项目。项目管理员可在“成员管理”中调整权限，各成员只看到自己加入的项目；当前共享内容仍为故事文档。
...
点击左上角项目名称，可切换已有项目。管理员可使用“新建项目”，只填写名称即可创建并切换到空白项目（初始不带配置表），先编写介绍、故事和配置数据。需要连接游戏工程时，再到“引擎设置”填写工程目录与枚举目录并点击“保存设置”。各项目的内容和引擎设置独立保存，后续修改目录不会更换项目存档，重启恢复上次选择。
...
在左侧“玩法核心”用图形整理游戏入口、模式分支和玩法循环。节点可拖动，连线记录操作与流转条件；双击循环模块进入内部流程，通过目录或面包屑返回上层。节点可引用已有玩法设计，支持持续经营等无终点循环。内容按项目保存，并随项目文件夹及 AI 文档导出。三个原型示例包含可展开的循环图，已有项目不会被示例更新覆盖。详见 [玩法核心](docs/gameplay-core.md)。
