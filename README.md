...
本机管理员的“管理”分组另有 **用户与权限** 模块，通过服务器管理员身份验证后可创建账号、启用／停用、重置密码、设置服务器角色，并配置自己管理的项目权限。团队连接使用账号密码输入。账号停用或密码重置会使旧会话失效，历史内容和本机草稿保留；系统保护最后一位有效管理员。用户与模块权限管理要求服务接口版本 6，升级后需重新连接团队。
...
管理员可在项目列表中右键点击本地项目，确认后从列表删除。项目存档和外部文件保留；删除最后一个项目后可在空工作区重新创建或导入。
