...
同一台服务器可保存多个协作项目。管理员从项目列表选择“新建协作项目”，使用团队 admin 连接后填写名称、配置成员权限，即可进入空白项目。项目管理员可在“成员管理”中调整权限，各成员只看到自己加入的项目。当前共享项目概览和故事文档，支持基本信息、独立里程碑、真实动态和成员列表；管理员／编辑者可编辑，只读成员可查看。
