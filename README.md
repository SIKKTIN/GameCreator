...
同一台服务器可保存多个协作项目。管理员从项目列表选择“新建协作项目”，使用团队 admin 连接后填写名称、配置成员权限，即可进入空白项目。项目管理员可在“成员管理”中调整权限，各成员只看到自己加入的项目。当前共享项目概览和故事文档，支持基本信息、独立里程碑、真实动态和成员列表。编辑者默认“概览只读、故事可编辑”，项目管理员可按项目单独授权或收回模块编辑权；只读成员只能查看。
...
选中本地项目后，管理员也可从项目列表选择“发布为协作项目”，预览并一次发布项目概览、里程碑和全部故事文档、设置协作名称和成员。原本地项目保留，其他模块留在本地；发布结果会被记录，重试或重启客户端后可进入已有协作项目，不覆盖团队修改。单次最多 500 篇故事、200 个里程碑、10 MiB。旧发布项目在团队概览尚未编辑时可从原发布入口一次性“补充项目概览”。项目概览要求服务接口版本 5，升级后重启服务并重新连接。
