...
以引擎为主工作目录时，先在“项目管理 → 项目启动”准备工程连接和制作人令牌，再初始化引擎协作入口。引擎 README 会指出绑定的 GameCreator 文件夹和本协议位置；开发者切换到该管理项目运行 `ai/submit-change.cjs`。初始化会更新管理项目的协作文件，并把成员凭证单独放入引擎所选入口的 `personal/`。完整流程与目录结构见 [项目启动](project-startup.md)。
